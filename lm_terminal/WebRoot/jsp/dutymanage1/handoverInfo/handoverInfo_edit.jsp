...
<body>
	<form id="handover_editform" class="form-horizontal" role="form">
	<input type="hidden" name="ha_Id" value="${handover.ha_Id }"/>
	<input type="hidden" id="us_Overid" name="us_Overid" value="${handover.us_Overer.us_Id }"/>
		<fieldset>
			<div class="form-group">
				<label class="col-sm-1 control-label" for="us_Overid">接班人</label>
				<div class="col-sm-4 input-message">
				    <div class='input-group'>
					<input type="tel" id="us_OverName" name="us_OverName" value="${handover.us_Overer.us_Name }" class="form-control" placeholder="点击选择接班人"
			...
			<div class="form-group">
				...
				<div class="col-sm-10">
					<textarea rows="8" cols="100" class="form-control" name="ha_Content" placeholder="交接事宜">${handover.ha_Content }</textarea>
				</div>
			</div>
		</fieldset>
	</form>
...
$(function(){
	$('#handover_editform').bootstrapValidator();
});
function handoverEdit_submitForm(index,window,id) {
	$('#handover_editform').bootstrapValidator('validate');
	if($('#handover_editform').data('bootstrapValidator').isValid()){
		$.post('dutymanage/handoverInfo/edit', $('#handover_editform').serialize(), function(j) {	
			if(j.success){
				window.$("#remove_handover"+id).remove();
				var str = '';
