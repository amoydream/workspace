...
<body>
	<div class="modal-body">
		<form id="person_addform" class="form-horizontal" role="form">
			<input type="hidden" id="or_id" name="or_id" value="${param.or_id }" />
			<input type="hidden" name="pe_del" value="0" />
			<fieldset>
				<div class="form-group">
					...
					<div class="col-sm-4 input-message">
						<input class="form-control" id="pe_name" name="pe_name" type="text" placeholder="输入姓名"
					...
					<div class="col-sm-4 input-message">
						<input type="hidden" id="po_id" name="po_ids" />
						<input class="form-control" id="po_name" name="po_name" type="text" placeholder="选择岗位"
							onclick="selectJobs();" data-bv-trigger="keyup" />
					</div>
				</div>
				<div class="form-group">
					<label class="col-sm-2 control-label" for="pe_sex">性别</label>
					<div class="col-sm-4 input-message">
						<input type="radio" name="pe_sex" value="F" checked="checked" />
						男
					...
					<div class="col-sm-4">
						<input type="radio" name="pe_type" value="0" checked="checked" />
						否
				...
				<div class="form-group">
					...
					<div class="col-sm-4 input-message">
						<input class="form-control" id="pe_identity" name="pe_identity" type="text"
							placeholder="输入身份证" pattern="(^\d{15}$)|(^\d{17}([0-9]|X)$)" />
					</div>
				</div>
				<div class="form-group">
				...
				<div class="form-group">
					...
					<div class="col-sm-10">
						<input class="form-control" id="pe_homeaddress" name="pe_homeaddress" type="text"
							placeholder="输入家庭住址" />
					</div>
				</div>
				<div class="form-group">
					...
					<div class="col-sm-10">
						<input class="form-control" id="pe_address" name="pe_address" type="text" placeholder="输入现住住址" />
					</div>
				</div>
				<div class="form-group">
					<label for="disabledSelect" class="col-sm-2 control-label">备注</label>
					<div class="col-sm-10">
						<input class="form-control" id="pe_remark" name="pe_remark" type="text" placeholder="输入备注" />
					</div>
				</div>
				<div class="form-group">
					<label class="col-sm-2 control-label" for="pe_leader">需报领导</label>
					<div class="col-sm-4">
						<input type="radio" name="pe_leader" value="0" checked="checked" />
						否
	...
	var ponames = [];
        ...
        $('#person_addform').bootstrapValidator();
        function personAdd_submitForm(index, window, or_id) {
	        $('#person_addform').bootstrapValidator('validate');
	        if($('#person_addform').data('bootstrapValidator').isValid()) {
		        $.post('work/person/add', $('#person_addform').serialize(), function(j) {
			        if(j.success) {
				        parent.layer.close(index);
				        window.postChild(or_id);
			        } else {
