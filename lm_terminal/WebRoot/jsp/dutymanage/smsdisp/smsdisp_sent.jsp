...
<body>
	<div style="margin-top: 10px; margin-left: 10px; margin-right: 10px;">
		<div style="margin-bottom: 10px;">
			<form id="searchForm" class="form-inline">
				...
				<div class="form-group">
					<a class="btn btn-primary" style="margin-left: 5px;" onclick="searchSent('1');"> <span
	...
	<script type="text/javascript">
		function viewsent(send_id) {
	        parent.parent.layer.open({
	            type : 2,
	            title : '查看短信',
	            area : ['1024px', '768px'],
	            scrollbar : false,
	            content : ['dutymanage/smsdisp/viewsent/' + send_id, 'no'],
	            btn : ['发送', '取消'],
        ...
        function searchSent(page) {
	        $('#page').val(page);
	        $.post('dutymanage/smsdisp/sent', $('#searchForm').serialize(), function(result) {
		        if(result.success) {
			        var str = '';
			        if(result.obj && result.obj.records) {
				        ...
				        for(var i = 0; i < records.length; i++) {
					        ...
					        str += '<td>';
					        str += '<a class="btn btn-xs btn-primary" onclick="viewsent(' + r.send_id
					               + ');"><span class="glyphicon glyphicon-comment"></span> 查看</a>';
					        str += '</td>';
					        str += '</tr>';
				        }
				        paginationNav('navbar', result.obj, 'searchSent');
			        }
