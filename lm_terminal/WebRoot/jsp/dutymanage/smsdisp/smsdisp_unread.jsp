...
<body>
	<div style="margin-top: 10px; margin-left: 10px; margin-right: 10px;">
		<div style="margin-bottom: 10px;">
			<form id="searchForm" class="form-inline">
				...
				<div class="form-group">
					<a class="btn btn-primary" style="margin-left: 5px;" onclick="searchUnread('1');"> <span
	...
	<script type="text/javascript">
		function viewunread(rece_id) {
	        parent.parent.layer.open({
	            type : 2,
	            title : '查看短信',
	            area : ['1024px', '768px'],
	            scrollbar : false,
	            content : ['dutymanage/smsdisp/viewunread/' + rece_id, 'no'],
	            btn : ['发送', '关闭'],
        ...
        function searchUnread(page) {
	        $('#page').val(page);
	        $.post('dutymanage/smsdisp/unread', $('#searchForm').serialize(), function(result) {
		        if(result.success) {
			        var str = '';
			        if(result.obj && result.obj.records) {
				        ...
				        for(var i = 0; i < records.length; i++) {
					        ...
					        str += '<td>';
					        str += '<a class="btn btn-xs btn-primary" onclick="viewunread(' + r.rece_id
					               + ');"><span class="glyphicon glyphicon-comment"></span> 查看</a>';
					        str += '</td>';
					        str += '</tr>';
				        }
				        paginationNav('navbar', result.obj, 'searchUnread');
			        }
			        $('#result').html(str);
		        } else {
