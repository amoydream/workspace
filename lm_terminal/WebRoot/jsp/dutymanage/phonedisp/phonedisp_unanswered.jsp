...
<div style="margin-bottom: 15px;">
	<form id="searchForm" class="form-inline">
		...
		<div class="form-group">
			<label for="vo_thatNo">电话号码</label>
...
<script type="text/javascript">
	function search(page) {
	    $('#page').val(page);
	    $.post('dutymanage/phonedisp/unanswered', $('#searchForm').serialize(), function(result) {
		    if(result.success) {
			    ...
			    for(var i = 0; i < records.length; i++) {
				    ...
				    str += '<td><a class="btn btn-xs btn-link" onclick="showHistory(\'' + r.vo_thatNo + '\');">'
				           + r.pe_name_ne + '&nbsp;<span class="badge">' + (r.grp_count < 100 ? r.grp_count : '99+')
				           + '</span></a><i class="' + (r.vo_callerFlag == '1' ? 'icon-reply' : 'icon-share-alt')
