...
<body>
	<div class="row" style="margin: 25px 10px 0 0;">
		<div class="col-md-7">
			<div style="margin-bottom: 10px;">
				<form class="form-inline" action="resource/assets/busstation"
					method="post">
					<input type="hidden" name="query" value="true" />
					<div class="form-group">
						<label for="bu_Name">名称</label> <input type="text" name="bu_Name"
							class="form-control" id="bu_Name" placeholder="输入广场名称">
					</div>
			...
			<!-- 庇护场所表格 -->
			<form id="busstationForm" action="resource/assets/busstation"
				method="post">
						...
						<c:forEach items="${pageView.records}" var="entry"
							...
							<c:choose>
								<c:when test="${statu.index % 2 ==0}">
									<tr id="mapRes${entry.bu_Id}"
										style="background-color: #ebf8ff;"
										onclick="trMapAnimation(${entry.bu_Id},${entry.bu_Longitude},${entry.bu_Latitude})">
										<input type="hidden" name="resId" value="${entry.bu_Id}">
										<input type="hidden" name="resName" value="${entry.bu_Name}">
										<input type="hidden" name="resAddress"
											value="${entry.bu_Address}">
										<input type="hidden" name="bu_Longitude"
											value="${entry.bu_Longitude}">
										<input type="hidden" name="bu_Latitude"
											value="${entry.bu_Latitude}">
										<td style="text-align:center">${entry.bu_Name}</td>
										<td style="text-align:center">${entry.bu_Linkman}</td>
										<td style="text-align:center">${entry.bu_Linkmantel}</td>
										<td style="text-align:center"><a
											href="javascript:void(0);" class="btn btn-primary btn-xs"
											onclick="resEdit(${entry.bu_Id})">编辑</a> <a
											href="javascript:void(0);" class="btn btn-danger btn-xs"
											onclick="resDelete(${entry.bu_Id})">删除</a></td>
									</tr>
								</c:when>
								<c:otherwise>
									<tr id="mapRes${entry.bu_Id}"
	...
	<script>
		...
		$("input[name^='bu_Longitude']").each(function(i, o) {
			longitude[i] = $(o).val();
		});
		$("input[name^='bu_Latitude']").each(function(i, o) {
			latitude[i] = $(o).val();
		});
		...
		function resAdd() {
			parent.parent.layer.open({
				...
				title : '添加汽车站',
				area : [ '800px', '640px' ],
				content : 'resource/assets/addip?type=8',
				btn : [ '确认', '取消' ],
				yes : function(index, layero) {
					var iframeWin = layero.find('iframe')[0].contentWindow
							.busstationAdd(index, window);
				},
			});
		}
		...
		function resEdit(resId) {
			parent.parent.layer.open({
				...
				title : '编辑汽车站',
				area : [ '800px', '640px' ],
				content : 'resource/assets/editip?type=8&id=' + resId,
				btn : [ '确认', '取消' ],
				yes : function(index, layero) {
					var iframeWin = layero.find('iframe')[0].contentWindow
							.busstationEdit(index, window);
				},
			});
		}

		function resDelete(resId) {
			layer.msg('确定删除该条汽车站信息？', {
				time : 0 //不自动关闭
				,
				btn : [ '确认', '取消' ],
				yes : function(index) {
					layer.close(index);
					$.post('resource/assets/delete?type=8&resId=' + resId,
							function(j) {
								if (j.success) {
