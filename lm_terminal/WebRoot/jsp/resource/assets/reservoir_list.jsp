...
<body>
	<div class="row" style="margin: 25px 10px 0 0;">
		<div class="col-md-7">
			<div style="margin-bottom: 10px;">
				<form class="form-inline" action="resource/assets/reservoir" method="post">
					<input type="hidden" name="query" value="true" />
					<div class="form-group">
						<label for="re_Name">名称</label> <input type="text" name="re_Name"
							class="form-control" id="re_Name" placeholder="输入水库名称">
					</div>
					...
					<button type="button" class="btn btn-primary" onclick="excelOut();">导出excel</button>
				</form>
			</div>

			<!-- 庇护场所表格 -->
			<form id="reservoirForm" action="resource/assets/reservoir" method="post">
				...
				<table
					...
					<tr class="info">
						...
						<th style="text-align:center">储水量</th>
					...
					<c:forEach items="${pageView.records}" var="entry"
						...
						<c:choose>
							<c:when test="${statu.index % 2 ==0}">
								<tr id="mapRes${entry.re_Id}"
									style="background-color: #ebf8ff;"
									onclick="trMapAnimation(${entry.re_Id},${entry.re_Longitude},${entry.re_Latitude})">
									<input type="hidden" name="resId" value="${entry.re_Id}">
									<input type="hidden" name="resName" value="${entry.re_Name}">
									<input type="hidden" name="resAddress"
										value="${entry.re_Address}">
									<input type="hidden" name="re_Longitude"
										value="${entry.re_Longitude}">
									<input type="hidden" name="re_Latitude"
										value="${entry.re_Latitude}">
									<td style="text-align:center">${entry.re_Name}</td>
									<td style="text-align:center">${entry.re_Area}</td>
									<td style="text-align:center">${entry.re_Capacity}</td>
									<td style="text-align:center">${entry.re_Linkman}</td>
									<td style="text-align:center">${entry.re_Linkmantel}</td>
									<td style="text-align:center">
											<a href="javascript:void(0);" class="btn btn-primary btn-xs"
												onclick="resEdit(${entry.re_Id})">编辑</a>
										
											<a href="javascript:void(0);" class="btn btn-danger btn-xs"
												onclick="resDelete(${entry.re_Id})">删除</a>
										</td>
								</tr>
							</c:when>
							<c:otherwise>
	...
	<script>
		...
		$("input[name^='re_Longitude']").each(function(i, o) {
			longitude[i] = $(o).val();
		});
		$("input[name^='re_Latitude']").each(function(i, o) {
			latitude[i] = $(o).val();
		});
		...
		function resAdd() {
			parent.parent.layer.open({
				...
				title : '添加水库',
				area : [ '800px', '640px' ],
				content : 'resource/assets/addip?type=5',
				btn : [ '确认', '取消' ],
				yes : function(index, layero) {
					var iframeWin = layero.find('iframe')[0].contentWindow.reservoirAdd(
							index, window);
				},
			});
		}
		...
		function resEdit(resId) {
			parent.parent.layer.open({
				...
				title : '编辑水库',
				area : [ '800px', '640px' ],
				content : 'resource/assets/editip?type=5&id=' + resId,
				btn : [ '确认', '取消' ],
				yes : function(index, layero) {
					var iframeWin = layero.find('iframe')[0].contentWindow
							.reservoirEdit(index, window);
				},
			});
		}

		function resDelete(resId) {
			layer.msg('确定删除该条水库信息？', {
				time : 0 //不自动关闭
				,
				btn : [ '确认', '取消' ],
				yes : function(index) {
					layer.close(index);
					$.post('resource/assets/delete?type=5&resId=' + resId, function(j) {
						if (j.success) {
		...
		function excelOut(){
			window.open('<%=basePath%>'+"resource/assets/reservoirExcelOut");
		}
	</script>
</body>
</html>