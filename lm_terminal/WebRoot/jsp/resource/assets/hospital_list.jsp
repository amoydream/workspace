...
<body>
	<div class="row" style="margin: 25px 10px 0 0;">
		<div class="col-md-7">
			<div style="margin-bottom: 10px;">
				<form class="form-inline" action="resource/assets/hospital"
					method="post">
					<input type="hidden" name="query" value="true" />
					<div class="form-group">
						<label for="ho_Name">名称</label> <input type="text" name="ho_Name"
							class="form-control" id="ho_Name" placeholder="输入医院名称">
					</div>
			...
			<!-- 庇护场所表格 -->
			<form id="hospitalForm" action="resource/assets/hospital"
				method="post">
				<input type="hidden" name="page" /> <input type="hidden"
					...
					<table
						...
						<tr class="info">
							...
							<th style="text-align:center">办公室电话</th>
						...
						<c:forEach items="${pageView.records}" var="entry"
							...
							<c:choose>
								<c:when test="${statu.index % 2 ==0}">
									<tr id="mapRes${entry.ho_Id}"
										style="background-color: #ebf8ff;"
										onclick="trMapAnimation(${entry.ho_Id},${entry.ho_Longitude},${entry.ho_Latitude})">
										<input type="hidden" name="resId" value="${entry.ho_Id}">
										<input type="hidden" name="resName" value="${entry.ho_Name}">
										<input type="hidden" name="resAddress"
											value="${entry.ho_Address}">
										<input type="hidden" name="ho_Longitude"
											value="${entry.ho_Longitude}">
										<input type="hidden" name="ho_Latitude"
											value="${entry.ho_Latitude}">
										<td style="text-align:center">${entry.ho_Name}</td>
										<td style="text-align:center">${entry.ho_Bednum}</td>
										<td style="text-align:center">${entry.ho_Officetel}</td>
										<td style="text-align:center">${entry.ho_Linkman}</td>
										<td style="text-align:center">${entry.ho_Linkmantel}</td>
										<td style="text-align:center"><a
											href="javascript:void(0);" class="btn btn-primary btn-xs"
											onclick="resEdit(${entry.ho_Id})">编辑</a> <a
											href="javascript:void(0);" class="btn btn-danger btn-xs"
											onclick="resDelete(${entry.ho_Id})">删除</a></td>
									</tr>
								</c:when>
								<c:otherwise>
									<tr id="mapRes${entry.ho_Id}"
	...
	<script>
		...
		$("input[name^='ho_Longitude']").each(function(i, o) {
			longitude[i] = $(o).val();
		});
		$("input[name^='ho_Latitude']").each(function(i, o) {
			latitude[i] = $(o).val();
		});
		...
		function resAdd() {
			parent.parent.layer.open({
				...
				title : '添加医院',
				area : [ '800px', '640px' ],
				content : 'resource/assets/addip?type=4',
				btn : [ '确认', '取消' ],
				yes : function(index, layero) {
					var iframeWin = layero.find('iframe')[0].contentWindow
							.hospitalAdd(index, window);
				},
			});
		}
		...
		function resEdit(resId) {
			parent.parent.layer.open({
				...
				title : '编辑医院',
				area : [ '800px', '640px' ],
				content : 'resource/assets/editip?type=4&id=' + resId,
				btn : [ '确认', '取消' ],
				yes : function(index, layero) {
					var iframeWin = layero.find('iframe')[0].contentWindow
							.hospitalEdit(index, window);
				},
			});
		}

		function resDelete(resId) {
			layer.msg('确定删除该条医院信息？', {
				time : 0 //不自动关闭
				,
				btn : [ '确认', '取消' ],
				yes : function(index) {
					layer.close(index);
					$.post('resource/assets/delete?type=4&resId=' + resId,
							function(j) {
								if (j.success) {
