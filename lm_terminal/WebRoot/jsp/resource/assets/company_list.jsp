...
<body>
	<div class="row" style="margin: 25px 10px 0 0;">
		<div class="col-md-7">
			<div style="margin-bottom: 10px;">
				<form class="form-inline" action="resource/assets/company"
					method="post">
					<input type="hidden" name="query" value="true" />
					<div class="form-group">
						<label for="co_Name">名称</label> <input type="text" name="co_Name"
							class="form-control" id="co_Name" placeholder="输入企业名称">
					</div>
			...
			<!-- 庇护场所表格 -->
			<form id="companyForm" action="resource/assets/company" method="post">
				<input type="hidden" name="page" /> <input type="hidden"
					...
					<table
						...
						<tr class="info">
							...
							<th style="text-align:center">主要产品</th>
						...
						<c:forEach items="${pageView.records}" var="entry"
							...
							<c:choose>
								<c:when test="${statu.index % 2 ==0}">
									<tr id="mapRes${entry.co_Id}"
										style="background-color: #ebf8ff;"
										onclick="trMapAnimation(${entry.co_Id},${entry.co_Longitude},${entry.co_Latitude})">
										<input type="hidden" name="resId" value="${entry.co_Id}">
										<input type="hidden" name="resName" value="${entry.co_Name}">
										<input type="hidden" name="resAddress"
											value="${entry.co_Address}">
										<input type="hidden" name="co_Longitude"
											value="${entry.co_Longitude}">
										<input type="hidden" name="co_Latitude"
											value="${entry.co_Latitude}">
										<td style="text-align:center">${entry.co_Name}</td>
										<td style="text-align:center">${entry.co_Product}</td>
										<td style="text-align:center">${entry.co_Linkman}</td>
										<td style="text-align:center">${entry.co_Linkmantel}</td>
										<td style="text-align:center"><a
											href="javascript:void(0);" class="btn btn-primary btn-xs"
											onclick="resEdit(${entry.co_Id})">编辑</a> <a
											href="javascript:void(0);" class="btn btn-danger btn-xs"
											onclick="resDelete(${entry.co_Id})">删除</a></td>
									</tr>
								</c:when>
								<c:otherwise>
									<tr id="mapRes${entry.co_Id}"
	...
	<script>
		...
		$("input[name^='co_Longitude']").each(function(i, o) {
			longitude[i] = $(o).val();
		});
		$("input[name^='co_Latitude']").each(function(i, o) {
			latitude[i] = $(o).val();
		});
		...
		function resAdd() {
			parent.parent.layer.open({
				...
				title : '添加企业',
				area : [ '800px', '640px' ],
				content : 'resource/assets/addip?type=7',
				btn : [ '确认', '取消' ],
				yes : function(index, layero) {
					var iframeWin = layero.find('iframe')[0].contentWindow
							.companyAdd(index, window);
				},
			});
		}
		...
		function resEdit(resId) {
			parent.parent.layer.open({
				...
				title : '编辑企业',
				area : [ '800px', '640px' ],
				content : 'resource/assets/editip?type=7&id=' + resId,
				btn : [ '确认', '取消' ],
				yes : function(index, layero) {
					var iframeWin = layero.find('iframe')[0].contentWindow
							.companyEdit(index, window);
				},
			});
		}

		function resDelete(resId) {
			layer.msg('确定删除该条企业信息？', {
				time : 0 //不自动关闭
				,
				btn : [ '确认', '取消' ],
				yes : function(index) {
					layer.close(index);
					$.post('resource/assets/delete?type=7&resId=' + resId,
							function(j) {
								if (j.success) {
