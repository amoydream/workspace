...
<body>
	<div class="row" style="margin: 25px 10px 0 0;">
		<div class="col-md-7">
			<div style="margin-bottom: 10px;">
				<form class="form-inline" action="resource/assets/bazaar"
					method="post">
					<input type="hidden" name="query" value="true" />
					<div class="form-group">
						<label for="ba_Name">名称</label> <input type="text" name="ba_Name"
							class="form-control" id="ba_Name" placeholder="输入市场名称">
					</div>
			...
			<!-- 庇护场所表格 -->
			<form id="bazaarForm" action="resource/assets/bazaar" method="post">
						...
						<c:forEach items="${pageView.records}" var="entry"
							...
							<c:choose>
								<c:when test="${statu.index % 2 ==0}">
									<tr id="mapRes${entry.ba_Id}"
										style="background-color: #ebf8ff;"
										onclick="trMapAnimation(${entry.ba_Id},${entry.ba_Longitude},${entry.ba_Latitude})">
										<input type="hidden" name="resId" value="${entry.ba_Id}">
										<input type="hidden" name="resName" value="${entry.ba_Name}">
										<input type="hidden" name="resAddress"
											value="${entry.ba_Address}">
										<input type="hidden" name="ba_Longitude"
											value="${entry.ba_Longitude}">
										<input type="hidden" name="ba_Latitude"
											value="${entry.ba_Latitude}">
										<td style="text-align:center">${entry.ba_Name}</td>
										<td style="text-align:center">${entry.ba_Workarea}</td>
										<td style="text-align:center">${entry.ba_Linkman}</td>
										<td style="text-align:center">${entry.ba_Linkmantel}</td>
										<td style="text-align:center"><a
											href="javascript:void(0);" class="btn btn-primary btn-xs"
											onclick="resEdit(${entry.ba_Id})">编辑</a> <a
											href="javascript:void(0);" class="btn btn-danger btn-xs"
											onclick="resDelete(${entry.ba_Id})">删除</a></td>
									</tr>
								</c:when>
								<c:otherwise>
									<tr id="mapRes${entry.ba_Id}"
	...
	<script>
		...
		$("input[name^='ba_Longitude']").each(function(i, o) {
			longitude[i] = $(o).val();
		});
		$("input[name^='ba_Latitude']").each(function(i, o) {
			latitude[i] = $(o).val();
		});
		...
		function resAdd() {
			parent.parent.layer.open({
				...
				title : '添加集贸市场',
				area : [ '800px', '640px' ],
				content : 'resource/assets/addip?type=2',
				btn : [ '确认', '取消' ],
				yes : function(index, layero) {
					var iframeWin = layero.find('iframe')[0].contentWindow
							.bazaarAdd(index, window);
				},
			});
		}
		...
		function resEdit(resId) {
			parent.parent.layer.open({
				...
				title : '编辑集贸市场',
				area : [ '800px', '640px' ],
				content : 'resource/assets/editip?type=2&id=' + resId,
				btn : [ '确认', '取消' ],
				yes : function(index, layero) {
					var iframeWin = layero.find('iframe')[0].contentWindow
							.bazaarEdit(index, window);
				},
			});
		}

		function resDelete(resId) {
			layer.msg('确定删除该条集贸市场信息？', {
				time : 0 //不自动关闭
				,
				btn : [ '确认', '取消' ],
				yes : function(index) {
					layer.close(index);
					$.post('resource/assets/delete?type=2&resId=' + resId,
							function(j) {
								if (j.success) {
