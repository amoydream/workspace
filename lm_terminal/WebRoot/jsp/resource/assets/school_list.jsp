...
<body>
	<div class="row" style="margin: 25px 10px 0 0;">
		<div class="col-md-7">
			<div style="margin-bottom: 10px;">
				<form class="form-inline" action="resource/assets/school" method="post">
					<input type="hidden" name="query" value="true" />
					<div class="form-group">
						<label for="sc_Name">名称</label> <input type="text" name="sc_Name"
							class="form-control" id="sc_Name" placeholder="输入庇护场所名称">
					</div>
			...
			<!-- 庇护场所表格 -->
			<form id="schoolForm" action="resource/assets/school" method="post">
					...
					<c:forEach items="${pageView.records}" var="entry"
						...
						<c:choose>
							<c:when test="${statu.index % 2 ==0}">
								<tr id="mapRes${entry.sc_Id}"
									style="background-color: #ebf8ff;"
									onclick="trMapAnimation(${entry.sc_Id},${entry.sc_Longitude},${entry.sc_Latitude})">
									<input type="hidden" name="resId" value="${entry.sc_Id}">
									<input type="hidden" name="resName" value="${entry.sc_Name}">
									<input type="hidden" name="resAddress"
										value="${entry.sc_Address}">
									<input type="hidden" name="sc_Longitude"
										value="${entry.sc_Longitude}">
									<input type="hidden" name="sc_Latitude"
										value="${entry.sc_Latitude}">
									<td style="text-align:center">${entry.sc_Name}</td>
									<td style="text-align:center">${entry.sc_Linkman}</td>
									<td style="text-align:center">${entry.sc_Linkmantel}</td>
									<td style="text-align:center">
											<a href="javascript:void(0);" class="btn btn-primary btn-xs"
												onclick="resEdit(${entry.sc_Id})">编辑</a>
											<a href="javascript:void(0);" class="btn btn-danger btn-xs"
												onclick="resDelete(${entry.sc_Id})">删除</a>
										</td>
								</tr>
							</c:when>
							<c:otherwise>
	...
	<script>
		...
		$("input[name^='sc_Longitude']").each(function(i, o) {
			longitude[i] = $(o).val();
		});
		$("input[name^='sc_Latitude']").each(function(i, o) {
			latitude[i] = $(o).val();
		});
		...
		function resAdd() {
			parent.parent.layer.open({
				...
				title : '添加学校',
				area : [ '800px', '640px' ],
				content : 'resource/assets/addip?type=1',
				btn : [ '确认', '取消' ],
				yes : function(index, layero) {
					var iframeWin = layero.find('iframe')[0].contentWindow.schoolAdd(
							index, window);
				},
			});
		}
		...
		function resEdit(resId) {
			parent.parent.layer.open({
				...
				title : '编辑学校',
				area : [ '800px', '640px' ],
				content : 'resource/assets/editip?type=1&id=' + resId,
				btn : [ '确认', '取消' ],
				yes : function(index, layero) {
					var iframeWin = layero.find('iframe')[0].contentWindow
							.schoolEdit(index, window);
				},
			});
		}

		function resDelete(resId) {
			layer.msg('确定删除该条学校信息？', {
				time : 0 //不自动关闭
				,
				btn : [ '确认', '取消' ],
				yes : function(index) {
					layer.close(index);
					$.post('resource/assets/delete?type=1&resId=' + resId, function(j) {
						if (j.success) {
		...
		function excelOut(){
			window.open('<%=basePath%>'+"resource/assets/schoolExcelOut");
		}
