...
<body>
	<div class="row" style="margin: 25px 10px 0 0;">
		<div class="col-md-7">
			<div style="margin-bottom: 10px;">
				<form class="form-inline" action="resource/assets/entertainment"
					method="post">
					<input type="hidden" name="query" value="true" />
					<div class="form-group">
						<label for="en_Name">名称</label> <input type="text" name="en_Name"
							class="form-control" id="en_Name" placeholder="输入场所名称">
					</div>
			...
			<!-- 庇护场所表格 -->
			<form id="entertainmentForm" action="resource/assets/entertainment"
				method="post">
						...
						<c:forEach items="${pageView.records}" var="entry"
							...
							<c:choose>
								<c:when test="${statu.index % 2 ==0}">
									<tr id="mapRes${entry.en_Id}"
										style="background-color: #ebf8ff;"
										onclick="trMapAnimation(${entry.en_Id},${entry.en_Longitude},${entry.en_Latitude})">
										<input type="hidden" name="resId" value="${entry.en_Id}">
										<input type="hidden" name="resName" value="${entry.en_Name}">
										<input type="hidden" name="resAddress"
											value="${entry.en_Address}">
										<input type="hidden" name="en_Longitude"
											value="${entry.en_Longitude}">
										<input type="hidden" name="en_Latitude"
											value="${entry.en_Latitude}">
										<td style="text-align:center">${entry.en_Name}</td>
										<td style="text-align:center">${entry.en_Address}</td>
										<td style="text-align:center">${entry.en_Linkman}</td>
										<td style="text-align:center">${entry.en_Linkmantel}</td>
										<td style="text-align:center"><a
											href="javascript:void(0);" class="btn btn-primary btn-xs"
											onclick="resEdit(${entry.en_Id})">编辑</a> <a
											href="javascript:void(0);" class="btn btn-danger btn-xs"
											onclick="resDelete(${entry.en_Id})">删除</a></td>
									</tr>
								</c:when>
								<c:otherwise>
									<tr id="mapRes${entry.en_Id}"
	...
	<script>
		...
		$("input[name^='en_Longitude']").each(function(i, o) {
			longitude[i] = $(o).val();
		});
		$("input[name^='en_Latitude']").each(function(i, o) {
			latitude[i] = $(o).val();
		});
		...
		function resAdd() {
			parent.parent.layer.open({
				...
				title : '添加娱乐场所',
				area : [ '800px', '640px' ],
				content : 'resource/assets/addip?type=9',
				btn : [ '确认', '取消' ],
				yes : function(index, layero) {
					var iframeWin = layero.find('iframe')[0].contentWindow
							.entertainmentAdd(index, window);
				},
			});
		}
		...
		function resEdit(resId) {
			parent.parent.layer.open({
				...
				title : '编辑娱乐场所',
				area : [ '800px', '640px' ],
				content : 'resource/assets/editip?type=9&id=' + resId,
				btn : [ '确认', '取消' ],
				yes : function(index, layero) {
					var iframeWin = layero.find('iframe')[0].contentWindow
							.entertainmentEdit(index, window);
				},
			});
		}

		function resDelete(resId) {
			layer.msg('确定删除该条娱乐场所信息？', {
				time : 0 //不自动关闭
				,
				btn : [ '确认', '取消' ],
				yes : function(index) {
					layer.close(index);
					$.post('resource/assets/delete?type=9&resId=' + resId,
							function(j) {
								if (j.success) {
