...
<body>
	<div class="row" style="margin: 25px 10px 0 0;">
		<div class="col-md-7">
			<div style="margin-bottom: 10px;">
				<form class="form-inline" action="resource/assets/supermarket" method="post">
					<input type="hidden" name="query" value="true" />
					<div class="form-group">
						<label for="su_Name">名称</label> <input type="text" name="su_Name"
							class="form-control" id="su_Name" placeholder="输入商场名称">
					</div>
			...
			<!-- 庇护场所表格 -->
			<form id="supermarketForm" action="resource/assets/supermarket" method="post">
					...
					<c:forEach items="${pageView.records}" var="entry"
						...
						<c:choose>
							<c:when test="${statu.index % 2 ==0}">
								<tr id="mapRes${entry.su_Id}"
									style="background-color: #ebf8ff;"
									onclick="trMapAnimation(${entry.su_Id},${entry.su_Longitude},${entry.su_Latitude})">
									<input type="hidden" name="resId" value="${entry.su_Id}">
									<input type="hidden" name="resName" value="${entry.su_Name}">
									<input type="hidden" name="resAddress"
										value="${entry.su_Address}">
									<input type="hidden" name="su_Longitude"
										value="${entry.su_Longitude}">
									<input type="hidden" name="su_Latitude"
										value="${entry.su_Latitude}">
									<td style="text-align:center">${entry.su_Name}</td>
									<td style="text-align:center">${entry.su_Linkman}</td>
									<td style="text-align:center">${entry.su_Linkmantel}</td>
									<td style="text-align:center">
											<a href="javascript:void(0);" class="btn btn-primary btn-xs"
												onclick="resEdit(${entry.su_Id})">编辑</a>
											<a href="javascript:void(0);" class="btn btn-danger btn-xs"
												onclick="resDelete(${entry.su_Id})">删除</a>
										</td>
								</tr>
							</c:when>
							<c:otherwise>
	...
	<script>
		...
		$("input[name^='su_Longitude']").each(function(i, o) {
			longitude[i] = $(o).val();
		});
		$("input[name^='su_Latitude']").each(function(i, o) {
			latitude[i] = $(o).val();
		});
		...
		function resAdd() {
			parent.parent.layer.open({
				...
				title : '添加商场',
				area : [ '800px', '640px' ],
				content : 'resource/assets/addip?type=3',
				btn : [ '确认', '取消' ],
				yes : function(index, layero) {
					var iframeWin = layero.find('iframe')[0].contentWindow.supermarketAdd(
							index, window);
				},
			});
		}
		...
		function resEdit(resId) {
			parent.parent.layer.open({
				...
				title : '编辑商场',
				area : [ '800px', '640px' ],
				content : 'resource/assets/editip?type=3&id=' + resId,
				btn : [ '确认', '取消' ],
				yes : function(index, layero) {
					var iframeWin = layero.find('iframe')[0].contentWindow
							.supermarketEdit(index, window);
				},
			});
		}

		function resDelete(resId) {
			layer.msg('确定删除该条商场信息？', {
				time : 0 //不自动关闭
				,
				btn : [ '确认', '取消' ],
				yes : function(index) {
					layer.close(index);
					$.post('resource/assets/delete?type=3&resId=' + resId, function(j) {
						if (j.success) {
