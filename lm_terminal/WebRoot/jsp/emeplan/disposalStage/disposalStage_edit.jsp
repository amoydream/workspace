...
<body>
<form id="disposalStage_editform" class="form-horizontal" role="form">
<input type="hidden" name="eds_id" value="${disposal_Stage.eds_id }"/>
		<fieldset>
			<div class="form-group">
				...
				<div class="col-sm-4 input-message">
				   <div class='input-group'>
					<input class="form-control" id="eds_name" name="eds_name" value="${disposal_Stage.eds_name }" type="text"
				...
				<div class="col-sm-4 input-message">
				   <div class='input-group'>
					<input class="form-control" id="eds_index" name="eds_index" value="${disposal_Stage.eds_index }" type="text"
			...
			<div class="form-group">
				...
				<div class="col-sm-9">
				    <textarea class="form-control" rows="3" id="eds_task" name="eds_task">${disposal_Stage.eds_task }</textarea>
				</div>
			</div>
			<div class="form-group">
				<label for="pt_remark" class="col-sm-2 control-label">备注</label>
				<div class="col-sm-9">
				    <textarea class="form-control" rows="2" id="eds_remark" name="eds_remark">${disposal_Stage.eds_remark }</textarea>
				</div>
			</div>
		</fieldset>
	</form>
<script type="text/javascript">
$('#disposalStage_editform').bootstrapValidator();
function disposalStageEdit_submitForm(index,window,id,level) {
	$('#disposalStage_editform').bootstrapValidator('validate');
	if($('#disposalStage_editform').data('bootstrapValidator').isValid()){
		$.post('emeplan/disposalStage/edit', $('#disposalStage_editform').serialize(), function(j) {	
			if(j.success){
				window.$("#disposalStageListsDel"+id).remove();
				var str = '';
