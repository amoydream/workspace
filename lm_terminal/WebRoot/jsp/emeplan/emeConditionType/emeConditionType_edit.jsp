...
<input type="hidden" name="eec_id" value="${conditionType.eec_id }"/>
<input type="hidden" name="pi_id" value="${conditionType.pi_id }"/>
		<fieldset>
		    <div class="form-group">
				<label for="eec_name" class="col-sm-2 control-label"><span style="color: red;">* </span>名称</label>
				<div class="col-sm-9 input-message">
				    <input class="form-control" id="eec_name" name="eec_name" value="${conditionType.eec_name }" type="text"
			...
			<div class="form-group">
				...
				<div class="col-sm-9">
				    <textarea class="form-control" rows="2" id="eec_desc" name="eec_desc">${conditionType.eec_desc }</textarea>
				</div>
			</div>
		</fieldset>
	</form>
...
function emeConditionTypeEdit_submitForm(index, window,id) {
	$('#emeConditionType_addform').bootstrapValidator('validate');
	if($('#emeConditionType_addform').data('bootstrapValidator').isValid()){
		$.post('emeplan/conditionType/edit', $('#emeConditionType_addform').serialize(), function(j) {	
			if(j.success){
				window.$("#emeConditionType"+id).remove();
				var str = '';
