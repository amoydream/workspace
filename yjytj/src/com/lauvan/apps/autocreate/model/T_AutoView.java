...
public class T_AutoView extends Model<T_AutoView> {
	...
	public void deleteView(T_AutoView t){
		...
		if(path!=null && !"".equals(path)){
			path = path.replace(".", "/");
			File file = new File(PathKit.getWebRootPath().replace("\\WebRoot", "")+"/src/"+path
					+"/"+controller.replaceFirst(controller.substring(0, 1), controller.substring(0, 1).toUpperCase())+".java");
			if(file.exists()){
				file.delete();
			}
		}
			...
			if(parent.isDirectory()){
				...
				if(files!=null && files.length>0){
					for(File f : files){
						...
						if("main.jsp".equals(name) || "add.jsp".equals(name)
								|| "edit.jsp".equals(name)){
							f.delete();
						}
					}
				}
