...
public class JudDeptController extends BaseController{
	...
	public void save(){
		...
		try {
			...
			if(success){
					renderText("{\"success\":true, \"msg\":\"保存成功！\",\"dialogid\":\"juddeptDialog\", \"gridid\":\"juddept_data\", \"treeObj\":\"juddepttree\",\"reloadid\":"+
									(getPara(0)==null?juddept.get("superdept_id"):getPara(0))+",\"idkey\":\"d_id\"}");
			}else{
	...
	public void getTreeData(){
		...
		List<Map<String, Object>> judList = new ArrayList<Map<String, Object>>();
		String sql = " 1=1";
		if(getPara(idKey)  != null && !"".equals(getPara(idKey))){
			sql += " and superdept_id =" + getPara("d_id");
		}else{
			Map<String, Object> root = new HashMap<String, Object>();
			root.put(idKey, "0");
			root.put("name", "司法行政机关");
			root.put(pidKey, "");
			judList.add(root);
			
		}
		sql += " order by deptid ";
		List<Record> list = Paginate.dao.getList("T_Bus_JudDept", sql);
