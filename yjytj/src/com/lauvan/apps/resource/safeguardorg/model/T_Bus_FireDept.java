...
public class T_Bus_FireDept extends Model<T_Bus_FireDept>{
	...
	public Page<Record> getPage(Integer pageSize, Integer pageNumber,
			Integer pid, String firedeptname, String levelcode) {
	...
	public String getChildIdsByDeptids(String deptids){
		String sql = "select wm_concat(f.deptid) from t_bus_firedept f start with f.deptid in ("
			+ deptids + ") connect by prior f.deptid = f.superdept_id";
		return Db.queryStr(sql);
	}
}
