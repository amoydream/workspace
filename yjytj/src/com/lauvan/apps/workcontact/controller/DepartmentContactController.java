 ...
 */
@RouteBind(path = "/Main/systemcontact/departmentcontact", viewPath = "/workcontact/baseworkcontact/systemcontact/departmentcontact")
	...
	public void importdepartcontactSave() {
		...
		try {
			...
			for (int i = 4; i < rows; i++) {
				...
				final int j = i;
				// 判断组织机构是否已存在
				T_Sys_Department depart = T_Sys_Department.dao.getDepartByName(
						sheet.getCell(0, i).getContents().toString(), null);
				if (depart != null) {
					T_Bus_ContactBook book = T_Bus_ContactBook.dao
							.getBookByDepartId(depart.getBigDecimal("D_ID"));
					// 如果组织机构已存在通讯信息则更新，否则添加
					if (book != null) {
						book.set("bo_worknumber", sheet.getCell(1, j)
								.getContents().toString());
						book.set("bo_fax", sheet.getCell(2, j).getContents()
								.toString());
						book.set("bo_email", sheet.getCell(3, j).getContents()
								.toString());
						book.set("bo_address", sheet.getCell(4, j)
								.getContents().toString());
						book.update();
						success++;
					} else {
						T_Bus_ContactBook book1 = new T_Bus_ContactBook();
						book1.set("bo_worknumber", sheet.getCell(1, j)
								.getContents().toString());
						book1.set("bo_fax", sheet.getCell(2, j).getContents()
								.toString());
						book1.set("bo_email", sheet.getCell(3, j).getContents()
								.toString());
						book1.set("bo_address", sheet.getCell(4, j)
								.getContents().toString());
			...
			result.put("msg", "应急机构通讯信息导入完成！数据总共有" + (error + success) + "行，导入成功"
					+ success + "行,系统组织机构不存部门" + error + "行!");
