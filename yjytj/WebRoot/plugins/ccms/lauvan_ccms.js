...
function OnCallOUT(CCID, CEID, CallID, ChannelNO) {
	...
	if($.messenger.msg(MSG_ID) == null) {
		...
		/*if(CCID == '' || CEID == '') {
			...
			caller = 'Y';
		} else if(!check_CCMS_SEAT(CCID) && !check_CCMS_TEL(CCID)) {
			caller = 'N';
		} else if(check_CCMS_TEL(CCID) && check_CCMS_SEAT(CEID)) {
			caller = 'N';
		} else if(check_CCMS_TEL(CEID) && !check_CCMS_SEAT(CCID)) {
			caller = 'N';
		} else if(check_CCMS_SEAT(CCID) && CEID == $.ccms.seatID) {
			caller = 'N';
		} else if(check_CCMS_SEAT(CEID) && CCID != $.ccms.seatID) {
			caller = 'N';
		} else {
			caller = 'Y';
		}*/
		...
		if(CCID == '' || CEID == '') {
			caller = '';
		} else if(CCID == CEID || CCID == $.ccms.seatID) {
			caller = 'Y';
		...
		if(caller == 'N') {
			...
			message += '<a href="javascript:void(0);" onclick="callout(\'' + CCID + '\')">';
			message += CCID;
			message += '</a></p></div>';
			$.messenger.post({
			    id : MSG_ID,
