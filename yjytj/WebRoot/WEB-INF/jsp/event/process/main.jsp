...
%>
	...
	$(function(){
		var attrArray ={ toolbar: [
				  ...
				  { text: '任务下达',title:'事件移动任务下达',iconCls: 'icon-phonego', permitParams:"${estatus=='00D' || pert:hasperti(applicationScope.event_apptask, loginModel.xdlimit)}",
					      dialogParams:{dialogId:'eventProcessDialog',href:basePath+"Main/eventProcess/appTask/${eventid}",width:700,
							height:500,formId:'eventProcessform',isNoParam:true}}              
                 ],
		fitColumns : true,
		idField:'ID',
		...
		var attrArray_app ={
				fitColumns : true,
				idField:'ID',
				url:basePath+"Main/eventProcess/getAppDataList/${eventid}",
				frozenColumns:[[]]
				};
		$.lauvan.dataGrid("_evappGrid",attrArray_app);
	...
	function faxView(faxid,taskid){
		if(taskid!=null && taskid!='' && taskid!='null'){
			...
			//下载传真文件
			window.open(basePath+"Main/communication/ccms/fax/download?fax_id="+faxid);
		}
	}
	function appFkFN(value,row,index){
		if(value!=null && value!='' && value!=undefined){
			var act = "<ul>";
			var fleng = row.FJLIST.length;
			for(var i=0;i<fleng;i++){
				act = act + "<li><a  href=\"javascript:void(0);\" onclick=appFJView('"+row.FJLIST[i].ID+"') >"+row.FJLIST[i].NAME+"</a></li>";
			}
			act = act+"</ul>"
			return act;
		}else{
	...
	function appFJView(id){
		$(document.body).append("<div id='evAppFjDialog'></div>");
		$("#evAppFjDialog").dialog({
			title:'事件移动反馈附件详情',
			width: 500,
			height: 400,
			modal: true,
 ...
 <div class="easyui-layout"  data-options="fit:true">
		...
		<div class="easyui-tabs" style="width:100%;" data-options="fit:true" >
			...
			<div title="事件移动反馈信息" >
				<table id="_evappGrid" cellspacing="0" cellpadding="0"> 
				    <thead> 
				        <tr> 
				            <th field="NAME" width="100">名称</th> 
				            <th field="ACCOUNT" width="100"  >号码</th>
				            <th field="BACKTIME" width="100"  >反馈时间</th>
				            <th field="BACKCONTENT" width="200"  >反馈内容</th>
				            <th field="ATTID" width="150" formatter="appFkFN">附件</th>
				        </tr> 
				    </thead> 
				</table>
			</div>
