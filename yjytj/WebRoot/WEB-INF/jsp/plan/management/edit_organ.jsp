...
function _planOrganClick(){
	...
	$("#_planOrgperDialog").dialog({
		...
		width: 800,
		height: 400,
		href: basePath+"Main/planMg/getOrganTree/${t.id}",
		buttons: [{text:'确定',
			iconCls:'icon-ok',
			handler:function(){
	 ...
	 <form id="planMgform" method="post" action="<%=basePath %>Main/planMg/save" style="width:100%;">
	    <input type="hidden" name="act" value="upd_organ"/>
	     <input type="hidden" name="t_Bus_PlanItem.pid" value="${t.pid}"/>
	      <input type="hidden" name="t_Bus_PlanItem.id" value="${t.id}"/>
	    <table class="sp-table" width="100%" cellpadding="0" cellspacing="0">
		    	...
		    	<td><input  name="t_Bus_PlanItem.itemname" type="text" class="easyui-textbox" data-options="required:true" value="${t.itemname}"  style="width: 300px;"/>  </td>
		    	</tr>
		    	<tr>
		  		<td class="sp-td1">机构/人员：</td>
		    	<td >
		    	<input type="hidden" name="organid" id="_planorganid" value="${t.itemid}"/>
		    	<input id="_planorganname" name="organname"  type="text" class="easyui-searchbox"   style="width: 180px;" 
		    	data-options="searcher:_planOrganClick,editable:false,icons:iconClear,value:'${organname}'"/>    
				</td>
		    	</tr>
		    	<tr>
		    	...
		    	<td >
		    		...
		    		data-options="validType:'length[0,200]'"  style="width:300px;height: 50px;">${t.itemcontent}</textarea> 
		    	</td>
		    	</tr>
	    </table>
    </form>