...
$.extend($.fn.validatebox.defaults.rules, {  
    checkSignCode:{//检查编码唯一性
        validator: function (value) {  
            var checkR=$.ajax({  
                ...
                cache : false,  
                type : 'post',    
                url : basePath+'Main/protectobjtable/check',    
                data : {    
                    'signcode' : value  
                }   
            }).responseText;    
    ...
    codeStyle: {// 校验编码格式
        validator: function (value) {
            return /^[a-zA-Z]([a-zA-Z0-9]|[_\w]){5,19}$/i.test(value);
        },
        message: '字母开头，允许6-20字节，允许字母数字_'
    }
});   
$(function(){
	var rulegrid;
	...
	rulegrid = $("#attrGrid").datagrid({
			 ...
			 columns: [[//显示的列
		                 { field: 'ATTRNAME', title: '属性名称', width: 150, sortable: true,
		                  ...
		                  { field: 'SQLTYPE', title: '属性类型', width: 100,
		                      editor: { type: 'combobox', options: { editable:false,required: true,valueField: 'label1',textField: 'value1',panelHeight:"auto",
		                  ...
		                  { field: 'ATTRSIZE', title: '属性大小', width: 150, sortable: true,
				                     editor: { type: 'textbox', options: {} }
				                 },
				                 { field: 'ISNULL', title: '是否非空', width: 100,
				                      editor: { type: 'combobox', options: { editable:false,valueField: 'label4',textField: 'value4',panelHeight:"auto",
				                	 data: [{label4: '1',value4: '是'},{label4: '0',value4: '否'}]
				                 }
				                 },formatter: function(value,row,index){
				                 if(value=='1'){
			                    	  return '是';
				                 ...
				                 { field: 'ISVIEW', title: '是否展示', width: 100,
				                      editor: { type: 'combobox', options: { editable:false,valueField: 'label5',textField: 'value5',panelHeight:"auto",
				                	 data: [{label5: '1',value5: '是'},{label5: '0',value5: '否'}]
				                 }
				                 }
				                 ,formatter: function(value,row,index){
					                 if(value=='1'){
			                ...
			                { field: 'SELCONTENT', title: '下拉列表字段', width: 200,
			                 ...
			                 { field: 'SELFALG', title: '是否读取参数', width: 100,
                     ...
                     $.messager.confirm("提示", "你确定要删除吗?", function (r) {
                         if (r) {
                           ...
                           //保存编辑内容到缓存中去
                             ...
                             rows.fname = fname;
                       	  $("#attrview").load(basePath+"Main/protectobjtable/attrView",rows);
                         }
                     });
                 }
                 else {
             ...
             { text: '确定', iconCls: 'icon-save', handler: function () {
                 ...
                 if(!flag && rdata.length>0){
	                 ...
	                 var attrArray={
	         				...
	         				width: 800,
	         				height: 500,
	         				href: basePath+"Main/protectobjtable/attrView?fname="+fname+"&rsize="+rows.length+rdata
	         		};
	  ...
	  <form id="proobjtable" method="post" action="<%=basePath%>Main/protectobjtable/save" style="width:100%;margin: 0 auto;padding: 0;">
	  <div data-options="region:'north',border:false" style="height:112px;">
	 
	    <table class="sp-table" width="100%" cellpadding="0" cellspacing="0">
		    	<tr>
		    	<td class="sp-td1">表单名称：</td>
		    	<td><input id="fname_ds"  name="t_Bus_TableForm.fname" type="text" class="easyui-textbox" data-options="required:true"  style="width: 300px;"/>  </td>
		    	
		  		<td class="sp-td1">表单编码：</td>
		    	<td >
		    	<input id="stime" name="t_Bus_TableForm.fcode" value="t_bus_exp_" type="text" class="easyui-textbox" data-options="required:true,validType:['checkSignCode','codeStyle']" style="width: 300px;"/>    
				</td>
		    	</tr>
		    	<tr>
		  		<td class="sp-td1">描述：</td>
		    	<td colspan="3">
		    		<textarea name="t_Bus_TableForm.remark" class="textbox easyui-validatebox" 
