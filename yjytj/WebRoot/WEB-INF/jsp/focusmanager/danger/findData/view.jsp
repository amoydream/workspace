	    ...
	    <table class="sp-table"  width="100%" cellpadding="0" cellspacing="0">
			...
			<c:if test="${attr.seq % 2==1 ||   attr.sqltype=='005'}"><tr style="height: auto;width: 100%;"></c:if>
				...
				<c:if test="${attr.sqltype=='011'}">
					 <td class="sp-td1" style="width: 150px;text-align: right;">${attr.remark}</td>
					  <td <c:if test="${(status.last == true || (status.last == false && alist[status.index+1].SQLTYPE=='005')) && attr.seq %2 ==1}" >colspan="3" style="text-align: left;"</c:if>>
					 <input type="text"  class="easyui-datebox" <c:if test="${attr.isnull=='1'}">data-options="required:true"</c:if> style="width:180px;" name="${attr.acode}"  id="${attr.acode}"    />
					  </td>
				</c:if>
				<c:if test="${attr.sqltype=='002'||attr.sqltype=='004'}">
					 <td class="sp-td1" style="width: 150px;text-align: right;">${attr.remark}</td>
					  <td <c:if test="${(status.last == true || (status.last == false && alist[status.index+1].SQLTYPE=='005')) && attr.seq %2 ==1}" >colspan="3" style="text-align: left;"</c:if>>
					  <input  id="${attr.acode}"  name="${attr.acode}" type="text" class="easyui-textbox" style="width:180px;" <c:if test="${attr.isnull=='1'}">data-options="required:true"</c:if>  <c:if test="${attr.defvalue=='001' ||attr.defvalue=='002'}">disabled="disabled"</c:if>   />
					  </td>
				</c:if>
				...
				<c:if test="${attr.sqltype=='005'}">
					 <td class="sp-td1" style="width: 150px;text-align: right;">${attr.remark}</td>
					  <td colspan="3" style="text-align: left;line-height: 21px;">
					 	<textarea  id="${attr.acode}"  rows="5" <c:if test="${attr.isnull=='1'}">data-options="required:true"</c:if> cols="85" name="${attr.acode}" class="textarea" >
					 	</textarea>
					  </td>
					  </tr>
				</c:if>
