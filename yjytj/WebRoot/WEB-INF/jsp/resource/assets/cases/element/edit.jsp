...
<%@ include file="/include/inc.jsp"%>
  ...
  $(function(){
	...
	var html = "<div id='fj_"+obj.id+"' ><input type='hidden' name='fjid' id='ELEfjval' value='" + obj.id +"'fjid' />";
...
<form id="elementForm" method="post"
	action="<%=basePath%>Main/caseselement/save" style="width:100%;">
	<table id="table" class="sp-table" width="100%" cellpadding="0"
		cellspacing="0">
		<input type="hidden" name="t_Bus_Cases_Element.ele_id" value="${model.ele_id }" />
		<tr>
			<td class="sp-td1">编号：</td>
			<td><input name="t_Bus_Cases_Element.code" value="${model.code}"
				type="text"
			...
			<td><input name="t_Bus_Cases_Element.content" value="${model.content}"
				type="text" class="easyui-textbox"
				data-options="prompt:'请输入要素标题',required:true,icons:iconClear" style="width:180px;" /></td>
		...
		<tr>
			...
			<td colspan="3"><textarea name="t_Bus_Cases_Element.elementdesc"
					...
					style="width: 560px;height: 50px;">${model.elementdesc}</textarea></td>
		...
		<tr>
				...
				<td colspan="5">
					<div>
						...
						<div id="ELEfileQueue">
							<div id="ELEfilebox" style="width:450px;">
								<c:if test="${! empty model.fjname}">
									<div id="ELEfj_${model.fjid}">
