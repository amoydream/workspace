...
$.extend({lauvan:{
	 ...
	 * **/
	openGisDialog:function(lng,lat,callback,options){
		...
		var attrArray={
			...
			if(options){
				attrArray=$.extend(false,attrArray,options);
			}
			$.lauvan.openCustomDialog("gisDialog",attrArray,callback,null);
	},
	drawPictureOnGisMap:function(list,layerId){
		var ifrName="overviewgis";
		if(window.frames[ifrName]){
			window.frames[ifrName].drawSign(list,layerId);
		}
	},
	focusPointOnGisMap:function(data,layerId){
		var ifrName="overviewgis";
		if(window.frames[ifrName]){
			window.frames[ifrName].focusAt(data,layerId);
		}
	}
}});