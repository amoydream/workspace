	    ...
	    <table class="sp-table"  width="100%" cellpadding="0" cellspacing="0">
			...
			<c:if test="${attr.seq % 2==1 ||   attr.sqltype=='005'}"><tr style="height: auto;width: 100%;"></c:if>
				<c:if test="${attr.sqltype=='001'}">
					 ...
					 <input type="text"  name="${attr.acode}" style="width:180px;" class="easyui-datetimebox"    />
					  </td>
				</c:if>
				<c:if test="${attr.sqltype=='011'}">
					 <td class="sp-td1" style="width: 150px;text-align: right;">${attr.remark}</td>
					  <td <c:if test="${(status.last == true || (status.last == false && alist[status.index+1].SQLTYPE=='005')) && attr.seq %2 ==1}" >colspan="3" style="text-align: left;"</c:if>>
					 <input type="text"  class="easyui-datebox" style="width:180px;" name="${attr.acode}"    />
					  </td>
				</c:if>
				<c:if test="${attr.sqltype=='002'||attr.sqltype=='004'}">
					 <td class="sp-td1" style="width: 150px;text-align: right;">${attr.remark}</td>
					  <td <c:if test="${(status.last == true || (status.last == false && alist[status.index+1].SQLTYPE=='005')) && attr.seq %2 ==1}" >colspan="3" style="text-align: left;"</c:if>>
					  <input name="${attr.acode}" type="text" class="easyui-textbox" style="width:180px;" <c:if test="${attr.defvalue=='001' ||attr.defvalue=='002'}">disabled="disabled"</c:if>   />
					  </td>
				</c:if>
				<c:if test="${attr.sqltype=='003'}">
					 <td class="sp-td1" style="width: 150px;text-align: right;">${attr.remark}</td>
						  <td <c:if test="${(status.last == true || (status.last == false && alist[status.index+1].SQLTYPE=='005')) && attr.seq %2 ==1}" >colspan="3" style="text-align: left;"</c:if>>
						 <select name="${attr.acode}" class="easyui-combobox" panelHeight="auto" style="width:180px;" >
					  ...
					  <td <c:if test="${(status.last == true || (status.last == false && alist[status.index+1].SQLTYPE=='005')) && attr.seq %2 ==1}" >colspan="3" style="text-align: left;"</c:if>>
					 	<c:choose>
					  	<c:when test="${attr.selfalg==1}">
					  		 <select id="${attr.acode}"  name="${attr.acode}" class="easyui-combobox" code="${attr.selcontent}" style="width:180px;" panelHeight="auto"></select>
					  	</c:when>
					  	<c:otherwise>
					  		<select id="${attr.acode}"  name="${attr.acode}" class="easyui-combobox" panelHeight="auto" style="width:180px;" >
					  		<option value="">请选择</option>
