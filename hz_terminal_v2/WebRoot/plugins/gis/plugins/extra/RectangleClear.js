...
(function() {
	 ...
	 var RectangleClear =
	 BMapLib.RectangleClear = function(map, opts){
            if (!map) {
                return;
            }
     ...
     * @return 无返回值
     */
    RectangleClear.prototype._setOptions = function(opts) {
        if (!opts) {
            return;
        }
        for (var p in opts) {
     ...
     *
     * @example <b>参考示例：</b><br />
     * myRectangleClearObject.setStrokeColor("#fff");
     */
    RectangleClear.prototype.setStrokeColor = function(color) {
     ...
     *
     * @example <b>参考示例：</b><br />
     * myRectangleClearObject.setLineStroke(3);
     */
    RectangleClear.prototype.setLineStroke = function(width) {
     ...
     *
     * @example <b>参考示例：</b><br />
     * myRectangleClearObject.setLineStyle("dashed");
     */
    RectangleClear.prototype.setLineStyle = function(style) {
     ...
     *
     * @example <b>参考示例：</b><br />
     * myRectangleClearObject.setOpacity(0.5);
     */
    RectangleClear.prototype.setOpacity = function(opacity) {
     ...
     *
     * @example <b>参考示例：</b><br />
     * myRectangleClearObject.setFillColor("#F0F");
     */
    RectangleClear.prototype.setFillColor = function(color) {
     ...
     *
     * @example <b>参考示例：</b><br />
     * myRectangleClearObject.setCursor("crosshair");
     */
    RectangleClear.prototype.setCursor = function(cursor) {
     ...
     * @return 无返回值
     */
    RectangleClear.prototype._updateStyle = function() {
        if (this._fDiv){
     ...
     *
     * @example <b>参考示例：</b><br />
     * myRectangleClearObject.getCursor();
     */
    RectangleClear.prototype.getCursor = function() {
     ...
     * @return 无返回值
     */
    RectangleClear.prototype._bind = function(){
        // 设置鼠标样式
     ...
     *
     * @example <b>参考示例：</b><br />
     * myRectangleClearObject.open();
     */
    RectangleClear.prototype.open = function() {
        // 判断拉框缩放状态是否已经开启
        ...
        var endDrawRect = function(e) {
            if (me._isOpen == true) {
				...
				var centerPoint,bounds,inbds,paths;
				var delLays=[],maskLays=[],i,len,v;
				for(i=0,len=overlays.length;i<len;i++){
					centerPoint=null;
					switch(overlays[i].toString()){
						case "[object Label]":
							v=overlays[i].V;
						    if(v.innerText==me._opts.followText)
								break;
							var p1=new BMap.Pixel(parseInt(v.style.left),parseInt(v.style.top)+v.offsetHeight),
								p2=new BMap.Pixel(parseInt(v.style.left)+v.offsetWidth,parseInt(v.style.top));
							bounds=new BMap.Bounds(map.pixelToPoint(p1), map.pixelToPoint(p2));
							inbds=bds.intersects(bounds);
							if(inbds)
								centerPoint=bounds.getCenter();
							
							break;
						case "[object Circle]":
							bounds=overlays[i].getBounds(),inbds=bds.intersects(bounds);
							if(inbds)
								centerPoint=overlays[i].getCenter();
							break;
						case "[object Polygon]":
						case "[object Polyline]":
							bounds=overlays[i].getBounds(),inbds=bds.intersects(bounds);
							if(inbds){
								paths=overlays[i].getPath();
								centerPoint=paths[0];
							}
							break;
						case "[object Marker]":
							centerPoint=overlays[i].getPosition();
							v=overlays[i].zc;
							if(v.innerHTML.indexOf("spotmkrs.png")!=-1|| !bds.containsPoint(centerPoint))
								centerPoint=null;
							break;
					...
					if(centerPoint && overlays[i].type && overlays[i].type=='sign'){
						var icon=new BMap.Icon(gisPath+"/css/icon/w_close_red.png",new BMap.Size(20,20)),
							marker=new BMap.Marker(centerPoint,{icon:icon});
							map.addOverlay(marker);
						maskLays.push(marker);
						delLays.push(overlays[i]);
					}
				}
				
				window.setTimeout(function(){
					if(delLays.length==0)return;
					if(confirm("您确定删除框选的标注吗？")){
						for(i=0,len<delLays.length;i<len;i++){
							map.removeOverlay(delLays[i]);
							map.removeOverlay(maskLays[i]);
						}
					}
					for(i=0,len=maskLays.length;i<len;i++)
							map.removeOverlay(maskLays[i]);
				},400);
 ...
 /**
     ...
     *
     * @example <b>参考示例：</b><br />
     * myRectangleClearObject.close();
     */
    RectangleClear.prototype.close = function() {
        if (!this._isOpen) {
            return;
        }
        this._isOpen = false;
     ...
     * @return 透明层的html字符串
     */
    RectangleClear.prototype._generateHTML = function() {
