 define([gisPath+"/plugins/coordtransform.js",
        ...
        "dojo/_base/array", "dojo/parser","esri/geometry/Extent","esri/geometry/Circle","esri/layers/ArcGISDynamicMapServiceLayer","esri/layers/ArcGISTiledMapServiceLayer","esri/dijit/Measurement","esri/units",
  	  ...
  	  "esri/tasks/FindTask", "esri/tasks/FindParameters","esri/geometry/Polyline","esri/geometry/Polygon","esri/geometry/Polyline",
        "dojo/domReady!"
    ], function(
    		coordtransform,Map,on,dom,lang,Color,Point,Draw,webMercatorUtils,FeatureLayer,GraphicsLayer, Legend,TextSymbol,Font,
    	      arrayUtils, parser,Extent,Circle,ArcGISDynamicMapServiceLayer,ArcGISTiledMapServiceLayer,Measurement,Units,SimpleMarkerSymbol,
    		  ...
    		  InfoTemplate,TooltipDialog, dijitPopup,FindTask,FindParameters,Polyline,Polygon,Polyline
    ) {
	 var customMap=function(containerId,lng,lat,zoom,gisUrl,basePath){
		 $('<div id="maskDiv" class="mask-loading">Loading...</div>').appendTo($(document.body));
		 //系数
		 var scaleWeight=0.4,zoomWeight=1,baseLayerName="baseLayer",drawToolbar;	
		 var eventHandler={},
		...
		/*begin*************定义相关函数******************/
			...
			var getAngle=function(sp,ep){
				var angle=0;
				if(sp==null || ep==null)
					angle=0;
				else if(ep.x==sp.x){
					angle=(ep.y>=sp.y)?90:270;
				}
				else{	
					var xl=(ep.y-sp.y)/(ep.x-sp.x);
					angle=Math.atan(xl)*180/Math.PI;
					if(xl<=0){
						if(ep.y>=sp.y)
							angle=-angle;
						else
	    ...
	    var map=new Map(containerId,{
			...
			this.addGraphicLayer=function(id){
				//添加标注图层
				var graphicLayer=new GraphicsLayer({id:id});
				...
				graphicLayer.on("visibility-change",function(obj){
					if(!obj.visible){
						for(var i=0,len=graphicLayer.graphics.length;i<len;i++)
							if(map.infoWindow.getSelectedFeature()==graphicLayer.graphics[i]){
								map.infoWindow.hide();
								break;
							}
					}
			...
			this.drawPicture=function(picSrc,picWidth,picHeight,mapPoint,attr,infoTemplate,graphicLayer){
			...
			this.drawText=function(txt,fontSize,color,point,offset,layer){
				var color2=Color.fromString(color);
				var font=new Font(fontSize,Font.STYLE_NORMAL,Font.VARIANT_NORMAL,Font.WEIGHT_BOLD,"宋体");
				var textSymbol=new TextSymbol(txt,font, color2);
				if(offset)
					textSymbol.setOffset(offset.x,offset.y);
				var graphic=new Graphic(point,textSymbol);
			...
			this.getPoint=function(lng,lat){
				var point=new Point(lng,lat,this.map.spatialReference);
			...
			//在地图画图形
			var _createDrawToolbar=function(drawEndHandler,type){
					...
					if(drawToolbar.EventListener)
						drawToolbar.EventListener.remove();
					var listen=drawToolbar.on("draw-end", drawEndHandler);
					drawToolbar.EventListener=listen;
					if(type){
						switch(type.toUpperCase()){
							case "FREEHAND_POLYLINE_ARROW":
							case "POLYLINE_ARROW":
								drawToolbar.extraType=type;
								type=type.substring(0,type.lastIndexOf("_"));
								break;
							default:
								drawToolbar.extraType=null;
						}
			...
			this.drawGeometry=function(geometry,layer,color){
				var color=Color.fromString(color),symbol;
				var color2=color;
				...
				switch(geometry.type){
					case "point":
						...
						if(data.type=='picture'){
							symbol=new PictureMarkerSymbol(basePath+data.url, data.width, data.height);
							symbol.setAngle(data.angle);
						}else if(data.type=='text'){
							var font=new Font(data.fontsize,Font.STYLE_NORMAL,Font.VARIANT_NORMAL,Font.WEIGHT_BOLD,"宋体");
							symbol=new TextSymbol(data.value,font, color);
							symbol.setOffset(15,15);
						}
						break;
					case "polyline":
						symbol=new SimpleLineSymbol(SimpleLineSymbol.STYLE_SOLID, color,4);
						break;
					default:
						color2.a=0.2;
			...
			this.newGeometry=function(obj){
				...
				switch(obj.type){
					...
					case "text":
						...
						break;
					case "extent":
						geometry=new Extent(obj.xmin,obj.ymin,obj.xmax,obj.ymax,this.map.spatialReference);
						break;
					case "polygon":
						geometry=new Polygon(obj.rings);
						break;
					case "polyline":
						geometry=new Polyline(obj.paths);
						break;
				}
				return geometry;
			}
			
			this.drawGraphical=function(type,color3,width,graphicLayer,isOnce){
				//添加标注图层
				...
				function addToMap(evt) {
				  ...
				  if(isOnce)
					  drawToolbar.deactivate();
				  map.showZoomSlider();
				  color=Color.fromString(color3);
				  switch (evt.geometry.type) {
					case "point":
					case "multipoint":
					  symbol = new SimpleMarkerSymbol(SimpleMarkerSymbol.STYLE_CIRCLE, 13,new SimpleLineSymbol(SimpleLineSymbol.STYLE_SOLID, color, width),color);
					  break;
					case "polyline":
					...
					default:
					  var color2=color;
					  color2.a=0.2;
					  symbol = new SimpleFillSymbol(SimpleFillSymbol.STYLE_SOLID, new SimpleLineSymbol(SimpleLineSymbol.STYLE_SOLID,color, width),color2);
					  break;
				  }
				  var graphic = new Graphic(evt.geometry, symbol);
				  graphicLayer.add(graphic);
				  
				  if(drawToolbar.extraType){
					  switch(drawToolbar.extraType){
						  	case "FREEHAND_POLYLINE_ARROW":
						  	case "POLYLINE_ARROW":
						  		var geo=evt.geometry;
						  		var index=geo.paths.length-1;
						  		var p1_index=0,p2_index=0,sp,ep,point;
						  		if(geo.paths[index].length>1){
						  			p1_index=geo.paths[index].length-2;
						  			p2_index=geo.paths[index].length-1;
						  		}
						  		sp=geo.paths[index][p1_index];
						  		ep=geo.paths[index][p2_index];
						  		point=me.getPoint(ep[0],ep[1]);
						  		sp=webMercatorUtils.lngLatToXY(sp[0], sp[1]);
						  		ep=webMercatorUtils.lngLatToXY(ep[0], ep[1]);
						  		var angle=getAngle({x:sp[0],y:sp[1]},{x:ep[0],y:ep[1]});
						  		
						  		var pictureSymbol= new PictureMarkerSymbol(basePath+"plugins/gis/css/img/redarrow.png", 18, 18);
				...
				_createDrawToolbar(addToMap,type);
				return drawToolbar;
			}

			this.getNewInfoTemplate=function(title,html){
				return infoTemplate=new InfoTemplate(title, html);
			}
			...
			*/
			...
			measurement=new Measurement({
				...
				lineSymbol:new SimpleLineSymbol(SimpleLineSymbol.STYLE_SOLID,new Color([255,0,0,1]),2),
				pointSymbol:new SimpleMarkerSymbol(SimpleMarkerSymbol.STYLE_CIRCLE,10
						,new SimpleLineSymbol(SimpleLineSymbol.STYLE_SOLID,new Color([255,0,0,1]),2),new Color([255,255,255,1]))
			 }, "measurementDiv");
			measurement.startup();
			measurement.hide();
			measurement.clearLayer=function(){
				measurement.clearResult();
				var layer=map.getLayer("measureLayer");
				if(layer)
					layer.clear();
			}
			measurement.on("measure-end", function(evt){
				var layerName="measureLayer";
				var layer=map.getLayer(layerName);
				if(evt.toolName=='distance'){
					var index=evt.geometry.paths[0].length-1;
					var x=evt.geometry.paths[0][index][0],y=evt.geometry.paths[0][index][1];
					me.drawText("总长："+evt.values.toFixed(4)+evt.unitName,14,"blue",me.getPoint(x,y),{x:20,y:-20},layer);
				}else if(evt.toolName=='area'){
					var index=Math.floor(evt.geometry.rings[0].length/2);
					var pt=evt.geometry.getPoint(0, index);
					me.drawText("面积："+evt.values.toFixed(4)+evt.unitName,14,"red",pt,{x:20,y:-20},layer);
				}
			...
			measurement.on("measure",function(evt){
				if(evt.toolName=='distance'){
					var distance=evt.values-(measurement.lastDistance ? measurement.lastDistance:0);
					measurement.lastDistance=evt.values;
					var layerName="measureLayer";
					var layer=map.getLayer(layerName);
					if(!layer){
						layer=me.addGraphicLayer(layerName);
					}
					
					me.drawText(distance.toFixed(4)+evt.unitName,14,"blue",evt.target._inputPoints[evt.target._inputPoints.length-1],{x:15,y:15},layer);
				}
			});
			measurement.on("measure-start",function(evt){
				measurement.lastDistance=0;
				var layerName="measureLayer";
				var layer=map.getLayer(layerName);
				if(!layer){
					layer=me.addGraphicLayer(layerName);
				...
				if(evt.toolName=='distance')
					me.drawText("起点",14,"blue",evt.target._inputPoints[evt.target._inputPoints.length-1],{x:10,y:10},layer);
			});
			...
			on(map,"load",function(){
				$(function(){
					...
					signDlg.appendToolBar("i_block","框选清除标注",function(){
						...
						_createDrawToolbar(findTarget,"EXTENT");
					});

					signDlg.appendToolBar("i_measure","测量",function(){
					
						var frame=signDlg.getFrame("i_measure");
						if(frame.length==0){
							frame=$("<div>");
							var picObj=[{"src":"/css/img/line.png","name":"测距","type":"polyline"},{"src":"/css/img/poly.png","name":"测面积","type":"POLYGON"}];
							...
							signDlg.appendFrame("i_measure",frame);
							frame.append(ul);
							
							ul.find("li").on("click",function(){
								//measurement.clearResult();
								measurement.clearLayer();
								switch($(this).find("img").attr("data-type").toLowerCase()){
								case "polyline":
									removeEventHandler("Map.click");
					...
					signBtn.onClick(function(){
						if(signDlg.getMainFrame().html()==""){
							...
							_createDrawToolbar(addToMap,null);
					...
					resourceBtn.onClick(function(){
						...
						if(mainFrame.html()==""){
							...
							$.post(basePath+"Main/geographic/overall/getResource",null,function(data){
								var setting= {
												...
												switch(nodeId.split("_")[0].toLowerCase()){
													...
													case "yjdw":
														...
														infoTemplate=new InfoTemplate("详细信息","<b>队伍名称: </b>\${NAME}<br>"
												                ...
												                + "<b><a href=\"javascript:void(0);\" onclick=\"_openDlg('"+basePath+"Main/team/getview/\${TEA_ID}')\">打开明细</a></b><br/>");
														break;
													//应急专家
													case "yjzj":icon="expert.png";lngAttr="LONGITUDE";latAttr="LATITUDE";
														infoTemplate=new InfoTemplate("详细信息","<b>专家姓名: </b>\${NAME}<br>"
												                ...
												                + "<b><a href=\"javascript:void(0);\" onclick=\"_openDlg('"+basePath+"Main/expert/getview/\${EX_ID}')\">打开明细</a></b><br/>");
														break;
													//应急避难所
													case "shtype":icon="refuge.gif";lngAttr="LONGITUDE";latAttr="LATITUDE";
														infoTemplate=new InfoTemplate("详细信息","<b>场所名称: </b>\${NAME}<br>"
												                ...
												                + "<b><a href=\"javascript:void(0);\" onclick=\"_openDlg('"+basePath+"Main/shelter/getview/\${SHE_ID}')\">打开明细</a></b><br/>");
														break;
													//应急设备
													...
													infoTemplate=new InfoTemplate("详细信息","<b>装备名称: </b>\${EQN_NAME}<br/>"
											                ...
											                + "<b>数量: </b>\${EQUIPNUM}<br/>"
											                + "<b>存放地址: </b>\${ADDRESS}<br/>"
											                + "<b><a href=\"javascript:void(0);\" onclick=\"_openDlg('"+basePath+"Main/equipstore/getview/\${EQS_ID}')\">打开明细</a></b><br/>");
														break;
													//应急案例
													case "evtp":icon="case.png";lngAttr="CAS_LONGITUDE";latAttr="CAS_LATITUDE";
														infoTemplate=new InfoTemplate("详细信息","<b>案例标题: </b>\${TITLE}<br>"
												                ...
												                + "<b>结束时间: </b>\${ENDTIME}<br>"
												                + "<b>地址: </b>\${ADDRESS}<br>"
												                + "<b><a href=\"javascript:void(0);\" onclick=\"_openDlg('"+basePath+"Main/cases/getview/\${CAS_ID}')\">打开明细</a></b><br/>");
														break;
													//物资仓库
													case "maleve":icon="house.png";lngAttr="LONGITUDE";latAttr="LATITUDE";
														infoTemplate=new InfoTemplate("详细信息","<b>名称: </b>\${NAME}<br>"
												                ...
												                + "<b><a href=\"javascript:void(0);\" onclick=\"_openDlg('"+basePath+"Main/repertory/getview/\${REP_ID}')\">打开明细</a></b><br/>");
														break;
												}
					...
					blueglassBtn.onClick(function(){
						...
						if(addFrame.length==0){
							var addFrame=$("<div>");
							var firstRow=$("<p style=\"font-weight:bold;display:inline-block;\">输入事故标题查询(为空查询全部)：</p>");
							var secondRow=$("<div style=\"margin:6px;\"></div>");
							var textInput=$("<input type=\"text\" style=\"width:270px;\"/>"),findBtn=$("<input type=\"button\" value=\"查询\"/>"),clearBtn=$("<input style=\"margin-left:4px;\" type=\"button\" value=\"清空\"/>");
							var thirdRow=$("<div  style=\"margin:18px 0;text-align:center;\"></div>");
							...
							findBtn.on("click",function(){
								var text=$.trim(textInput.val());
									$.post(basePath+"Main/eventSearch/getContent",{ename:text,flag:1},function(data){
										...
										var i,len,eventName,address,newObj,centerPoint;
										ul.empty().append(li);
										...
										for(i=0,len=data.length;i<len;i++){
											...
											var pictureSymbol= new PictureMarkerSymbol(gisPath+"/css/img/donghua.gif", 32, 32),
											  point=new Point(data[i].EV_LONGITUDE,data[i].EV_LATITUDE,map.spatialReference);
											if(map.spatialReference.isWebMercator()){
												point=webMercatorUtils.geographicToWebMercator(point);
											}
											...
											var infoTemplate=new InfoTemplate("详细信息","<b>事件名称: </b>\${EV_NAME}<br>"
								                ...
								                + "<b>事发时间: </b>\${EV_DATE}<br>"
								                +"<b><a href=\"javascript:void(0);\" onclick=\"_openDlg('"+basePath+"Main/event/view/\${ID}')\">打开明细</a></b><br/>"
								                +(data[i].EV_STATUS=='00C'?"<a href='"+basePath+"Main/geographic/dispatch/main?eventid=\${ID}'>跳转指挥调度</a>":"<span style='color:red;'>未启动预案</span>")+"<br/>");

											var graphic=new Graphic(point,pictureSymbol,data[i],infoTemplate);
											graphicLayer.add(graphic);
											if(!centerPoint){
												centerPoint=point;
											}
											...
											newObj.on("click",function(){
												...
												var value=$.parseJSON($(this).attr("data-value"));
												var point=new Point(value.EV_LONGITUDE,value.EV_LATITUDE,map.spatialReference);
												if(map.spatialReference.isWebMercator()){
													point=webMercatorUtils.geographicToWebMercator(point);
												}
												map.centerAt(point);
					...
					userBtn.onClick(function(){
						...
						if(mainFrame.html()==""){
							...
							var pointLayer=addGraphicLayer(map,userDlg,"userPLayer");
							
							var getRealTimeGPS=function(){
								$.post(basePath+"Main/gpsinfo/getRealTimeGPS",null,function(data){
									pointLayer.clear();
									for(var i=0,len=data.length;i<len;i++){
										var pt=me.getPoint(data[i].POSX,data[i].POSY);
										var template=me.getNewInfoTemplate("明细","<b>\${REALNAME}</b>");
										me.drawPicture(basePath+"plugins/gis/css/img/donghua.gif",25,25,pt,data[i],template,pointLayer);
									}
								});
							}
							...
							userDlg.addCloseEvent("layerHide_udgps",function(){window.clearInterval(timer);});
							userDlg.addOpenEvent("layerShow_udgps",function(){timer=window.setInterval(getRealTimeGPS,10000);});
					...
					systemBtn.onClick(function(){
						...
						if(mainFrame.html()==""){
							...
							$.post(basePath+"Main/geographic/overall/getCommEqu",null,function(data){
								var setting= {
										view: {
								      selectedMulti: false,
								      nameIsHTML: true
									...
									for(i=0,len=nodes.length;i<len;i++){
										...
										if(!nodes[i].root && !flag){
											...
											var infoTemplate=new InfoTemplate("详细信息","<b>设备名称: </b>\${name}<br>"
								                ...
								                + "<b>设备IP: </b>\${ip}<br>"
								                + "<b>设备端口: </b>\${port}<br>"
								                +(parentNode.id=="0001"?"<b><a href='javascript:void(0);' onclick=\"_openMedia('"+basePath+"upload/video.mp4');\">打开明细</a></b>":""));
					...
					aboutBtn.onClick(function(){
						...
						if(mainFrame.html()==""){
							var h2=$("<h2>惠州离线GIS模型</h2>"),h3=$("<h3 style='color:#1196EE;'>v1.0版本</h3>"),
					...
					hideBtn.onClick(function(){
						...
						if(me.is(":hidden"))
							...
							me.hide();
						*/
						var featureLayer = new FeatureLayer("http://192.168.0.180:6080/arcgis/rest/services/huizhou/MapServer/19");
						map.addLayer(featureLayer);
						
						var se=featureLayer.getSelectedFeatures();
						debugger;
					});
				});
