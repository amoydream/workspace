...
public class UserContactController extends BaseController {
	...
	// 获取岗位类型树
		public void getTypeTree() {
			String jsonStr = "[]";
			try {
				List<Record> list = T_Sys_Parameter.dao.getParamByCode("EMPOSITION",
						true);
	...
	public void getBookGridData() {
		...
		String sql = "t_sys_user u inner join t_bus_contactbook b on b.bo_userid = u.user_id where u.dept_id ="
				+ pid;
		// 获取表格表页数据
		Page<Record> page = Paginate.dao.getPage(pageSize, pageNumber, sql,
				null, null);
		List<Record> list = page.getList();
		int totalCount = page.getTotalRow();
		// 调用JsonUtil函数返回datagrid表格json数据
	...
	public void importusercontactSave() {
		...
		try {
			...
			for (int i = 4; i < rows; i++) {
				...
				final int j = i;
				// 判断用户是否已存在
				boolean fuser = T_Sys_User.dao.ifExsitAccount(
						sheet.getCell(0, i).getContents().toString(),
						loginModel.getUserId().intValue());
				if (fuser) {
					T_Sys_User user = T_Sys_User.dao.getUserByAccount(sheet
							.getCell(0, i).getContents().toString());
					T_Bus_ContactBook book = T_Bus_ContactBook.dao
							.getBookByUserId(user.getBigDecimal("USER_ID"));
					...
					if (book != null) {
						...
						book.set("bo_worknumber", sheet.getCell(2, j).getContents()
								.toString());
						book.set("bo_mobile", sheet.getCell(3, j).getContents()
								.toString());
						book.set("bo_homenumber", sheet.getCell(4, j)
								.getContents().toString());
						book.set("bo_fax", sheet.getCell(5, j).getContents()
								.toString());
						book.set("bo_email", sheet.getCell(6, j).getContents()
								.toString());
						book.set("bo_address", sheet.getCell(7, j)
								.getContents().toString());
						book.update();
						success++;
					} else {
						...
						if (StringUtils.isNotBlank(aname)) {
							T_Sys_Parameter param1 = T_Sys_Parameter.dao.getByCode(
									sheet.getCell(1, i).getContents().toString(),
									"EMPOSITION");
							if (param1 != null) {
						...
						book1.set("bo_worknumber", sheet.getCell(2, j)
								.getContents().toString());
						book1.set("bo_mobile", sheet.getCell(3, j)
								.getContents().toString());
						book1.set("bo_homenumber", sheet.getCell(4, j)
								.getContents().toString());
						book1.set("bo_fax", sheet.getCell(5, j).getContents()
								.toString());
						book1.set("bo_email", sheet.getCell(6, j).getContents()
								.toString());
						book1.set("bo_address", sheet.getCell(7, j)
								.getContents().toString());
			...
			result.put("msg", "系统应急人员用户通讯信息导入完成！数据总共有" + (error + success) + "行，导入成功"
					+ success + "行,不存在的系统应急人员用户" + error + "行!");
