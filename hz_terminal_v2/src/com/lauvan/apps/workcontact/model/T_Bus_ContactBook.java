...
public class T_Bus_ContactBook extends Model<T_Bus_ContactBook> {
	...
	public T_Bus_ContactBook getBookByUserId(BigDecimal user_id) {
		return dao.findFirst(
				"select * from t_bus_contactbook where bo_userid=?", user_id);
	}
	...
	public T_Bus_ContactBook getBookByDepartId(BigDecimal d_id) {
		return dao.findFirst(
				"select * from t_bus_contactbook where bo_deptid=?", d_id);
	}
	 ...
	 *            IDs
	 * */
	public Page<Record> getBookAndUserPage(Integer pageSize,
			Integer pageNumber, String ids) {
		String select = "select * ";
		StringBuffer str = new StringBuffer();
		str.append("from t_bus_contactbook b ");
		if (ids != null && !"".equals(ids)) {
			str.append(" ,t_sys_user u where b.bo_userid=u.user_id and b.bo_id in("
					+ ids + ")");
		}
	...
	public boolean deleteForDepart(Integer[] ids) throws Exception {
		try {
			...
			for (Integer id : ids) {
				...
				T_Bus_ContactBook book = dao.getBookByDepartId(depart
						.getBigDecimal("D_ID"));
				if (book != null) {
					book.delete();
					flag = true;
