...
public class T_Bus_EventInfo extends Model<T_Bus_EventInfo> {
	...
	public List<Record> getEventList(String ids, String model) {
		String sql = "select ei.ev_name,ei.ev_reportdate,ei.ev_reporttel,(select p1.p_name from t_sys_parameter p1,t_sys_parameter p2 where " + "p1.sup_id=p2.id and p1.p_acode=ei.EV_REPORTMODE and p2.p_acode='EVRP') as reportmode," + "(select p3.p_name from t_sys_parameter p3,t_sys_parameter p4 where p3.sup_id=p4.id and p3.p_acode=ei.ev_type and p4.p_acode='EVTP') as evtp" + ",o.d_name,ars.p_name as area from t_bus_eventinfo ei left join t_sys_department o on ei.organid = o.d_id left join (select a.* from t_sys_parameter a where " + "a.p_acode<>'EVQY' Start With a.p_acode='EVQY' Connect By Prior a.id = a.sup_id) ars on ei.OCCURAREA=ars.p_acode where ei.ev_reportmode='" + model + "' and ei.id in(" + ids + ") order by ei.ev_date ASC";
		return Db.find(sql);
	}
	...
	public List<Record> getListByName(String name, String flag) {
		...
		sql = sql + " order by decode(t.ev_status,'00C',1,'00B',2,'00A',3,'00D',4) ";
		return Db.find(sql);
	}
	...
	//根据事件性质及状态查找列表
	public List<T_Bus_EventInfo> getListByStateStatus(String state, String status) {
		String sql = "select e.*,decode(e.ev_type,null,'',(select a.p_name from t_sys_parameter a where a.p_acode=e.ev_type" + " Start With a.p_acode='EVTP' Connect By  Prior a.id = a.sup_id  )) as evtype_name,decode(e.ev_type,null,'',(select a.p_name from t_sys_parameter a where a.p_acode=e.ev_level" + " Start With a.p_acode='EVLV' Connect By  Prior a.id = a.sup_id  ))  as evlevel_name , " + " (select count(p.id) from  t_bus_presch_instance p where p.event_id = e.id and p.instflag is null) as num" + " from t_bus_eventinfo e  where  e.ev_state = ? and e.ev_status in (" + status + ") ";
		return dao.find(sql, state);
	}

	public List<Record> getEventList(String ids) {
		String sql = "select ei.ev_name,ei.ev_reportdate,ei.ev_reporttel,(select p1.p_name from t_sys_parameter p1,t_sys_parameter p2 where " + "p1.sup_id=p2.id and p1.p_acode=ei.EV_REPORTMODE and p2.p_acode='EVRP') as reportmode," + "(select p3.p_name from t_sys_parameter p3,t_sys_parameter p4 where p3.sup_id=p4.id and p3.p_acode=ei.ev_type and p4.p_acode='EVTP') as evtp" + ",o.d_name,ars.p_name as area from t_bus_eventinfo ei left join t_sys_department o on ei.organid = o.d_id left join (select a.* from t_sys_parameter a where " + "a.p_acode<>'EVQY' Start With a.p_acode='EVQY' Connect By Prior a.id = a.sup_id) ars on ei.OCCURAREA=ars.p_acode where ei.id in(" + ids + ")";
		return Db.find(sql);
	}
	...
	public List<Record> getEVtjList(String stime, String etime) {
		String sql = "select t.*,decode(t.ev_bssx,null,null,(select p1.p_name from t_sys_parameter p1,t_sys_parameter p2" 
			...
			+",(select d_name from t_sys_department where d_id = e.organid)) as organname,e.ev_name as title,e.ev_bssx"
			...
			+ ",e.id from t_bus_eventinfo e  where  e.ev_date <='" + etime + "' and e.ev_date>='" + stime +"'"
			+" and e.ev_state='00X' "
			+ ") t where t.organname is not null order by organname desc";
		return Db.find(sql);
	}
}
