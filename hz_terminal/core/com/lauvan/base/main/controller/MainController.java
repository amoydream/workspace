...
public class MainController extends Controller {
	...
	public void index(){
		...
		if(plist!=null && plist.size()>0){
			for(T_Sys_Parameter p : plist){
				...
				if((rootCode+"_sysvoice").equals(pname)){
					setAttr("_snoteVoice",pacode);
				}
			}
		}
	...
	public void systemSetSave(){
		try {
			...
			T_Sys_Parameter t7 = T_Sys_Parameter.dao.getByCode( rootCode+"_sysvoice","SYSSET");
			if(t7!=null){
				t7.set("p_acode",getPara("sysvoice"));
				t7.update();
			}else{
				t7 = new T_Sys_Parameter();
				t7.set("p_name", rootCode+"_sysvoice");
				t7.set("p_acode", getPara("sysvoice"));
				t7.set("sup_id", sys.get("id"));
				T_Sys_Parameter.dao.insert(t7);
			}
