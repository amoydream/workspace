...
public class T_Sys_Department extends Model<T_Sys_Department> {
	...
	public List<Record> getDepartmentExtList(String deptId, String extId) {
		String sql = "";
		if("oracle".equals(JFWebConfig.attrMap.get("dbaType"))) {
			sql = "select a.* from t_sys_department a Start With a.d_pid =? and a.d_id<>" + extId + " Connect By  Prior a.d_id = a.d_pid and a.d_id<>" + extId;
	...
	public T_Sys_Department getDepartByName(String name, String supcode) {
		...
		if(supcode != null && !"".equals(supcode)) {
			if("oracle".equals(JFWebConfig.attrMap.get("dbaType"))) {
				sql = sql + " and d.d_id in (select a.d_id from t_sys_department a Start With a.d_id =" + supcode + " Connect By  Prior a.d_id = a.d_pid)";
	...
	public String getSeatIds(Number id) {
		return Db.queryStr("SELECT SEATIDS FROM T_SYS_DEPARTMENT WHERE D_ID = " + id);
	}

	public T_Sys_Department getuserDept(String userid) {
		String sql="select d.* from t_sys_user u,t_sys_department d where u.user_id="+userid+" and d.d_id=u.dept_id";
		return dao.findFirst(sql);
	}
}
