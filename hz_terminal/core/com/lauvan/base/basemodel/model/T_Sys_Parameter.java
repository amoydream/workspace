...
public class T_Sys_Parameter extends Model<T_Sys_Parameter> {
	 ...
	 * 
	 * **/
	public T_Sys_Parameter getPcodeRoot(String pcode,String rootcode){
		String sql = "select p.* from (select * from t_sys_parameter p1  start with p1.p_acode='"
					+pcode+"' connect by prior p1.sup_id = p1.id ) p where p.sup_id=(select id from t_sys_parameter where p_acode='"
					+rootcode+"')";
		return dao.findFirst(sql);
	}
}
