...
public class PermitInterceptor implements Interceptor {
	...
	public void intercept(Invocation ai) {
		...
		if(requrl.endsWith(".js") || requrl.endsWith(".gif") || requrl.endsWith(".jpg") || requrl.endsWith(".png") || requrl.endsWith(".css") || requrl.endsWith(".html") || requrl.endsWith(".htm") || requrl.endsWith(".xml") || requrl.endsWith(".swf") || requrl.endsWith(".htc") || requrl.endsWith(".cab")) {
			ai.invoke();
			return;
		}
		if(!requrl.startsWith("/Main") && !requrl.startsWith("/main") && !requrl.equals("/Login/doLogin")) {
			ai.invoke();
			return;
		}
		if(null == ai.getController().getSession().getAttribute("loginModel")) {
			...
			try {
				...
				if(ai.getActionKey().equals("/Main")) {
		...
		if(actionKey.equals("/Main") || actionKey.equals("/Main/getMenu") || actionKey.equals("/Main/menu") || actionKey.equals("/Main/pwordset") || actionKey.equals("/Main/pwordSave") || actionKey.equals("/Main/systemSet") || actionKey.equals("/Main/systemSetSave") || actionKey.equals("/Login/logout")) {
			ai.invoke();
			return;
		}
		String methodname = ai.getMethodName();

		if(methodname.startsWith("get") || methodname.equals("save") || methodname.endsWith("Save")) { //以get开头的方法或者save方法直接通过部拦截
			ai.invoke();
			return;
		}
		...
		if(!loginModel.getIsAdmin() && !loginModel.getIsSuper()) {
			...
			//判断模块是否有权限
			if(!PermitHandler.haspermit(moduleId, loginModel.getLimit()) && "0".equals(module.getStr("modeltype"))) {
			...
			//判断功能点是否有权限
			if(!PermitHandler.haspermit(moduleId, loginModel.getXdlimit(), module.getNumber("p_id").intValue(), loginModel.getLimit()) && "1".equals(module.getStr("modeltype"))) {
