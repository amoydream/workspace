...
public class CcmsController extends BaseController {
	...
	public V_Contact getContact(String TEL_NUMBER) {
		...
		if(StringUtils.trimToNull(TEL_NUMBER) != null) {
			...
			for(V_Contact c : CONTACT_CACHE) {
				String _TEL_NUMBER = StringUtils.trimToEmpty(c.getStr("TEL_NUMBER"));
	...
	private boolean checkTEL(String TEL_NUMBER, String TEL_NUMBERS) {
		if("".equals(TEL_NUMBERS)) {
			return false;
		}
		if(TEL_NUMBER.equals(TEL_NUMBERS)) {
			return true;
		}
		TEL_NUMBER = StringUtils.reverse(TEL_NUMBER);
		String TEL_NUMBER_7 = TEL_NUMBER;
		String TEL_NUMBER_8 = TEL_NUMBER;
		if(TEL_NUMBER.length() > 8 && TEL_NUMBER.startsWith("0")) {
			TEL_NUMBER_7 = TEL_NUMBER.substring(0, 7);
			TEL_NUMBER_8 = TEL_NUMBER.substring(0, 8);
		}

		String _TEL_NUMBER = null;
		String _TEL_NUMBER_7 = null;
		String _TEL_NUMBER_8 = null;
		String[] TEL_NUMBER_ARR = TEL_NUMBERS.split(",");
		for(int i = 0; i < TEL_NUMBER_ARR.length; i++) {
			_TEL_NUMBER = StringUtils.reverse(TEL_NUMBER_ARR[i]);
			...
			if(TEL_NUMBER_7.equals(_TEL_NUMBER_7) || TEL_NUMBER_8.equals(_TEL_NUMBER_8)) {
				return true;
			}
		}
