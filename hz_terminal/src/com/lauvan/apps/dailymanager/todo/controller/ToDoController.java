...
public class ToDoController extends BaseController {
	...
	public void getallGridData() {
		...
		page = T_ThingInfo.dao.getallGridPage(pageNumber, pageSize, code, name, loginModel, null);
	...
	public void getneedGridData() {
		...
		Page<Record> page;
		LoginModel loginModel = getSessionAttr("loginModel");
		page = T_ThingInfo.dao.getallGridPage(pageNumber, pageSize, code, name, loginModel, "0");
		List<Record> list = page.getList();
		int totalCount = page.getTotalRow();
		// 调用JsonUtil函数返回datagrid表格json数据
		String jsonStr = JsonUtil.getGridData(list, totalCount);
	...
	public void gethaveGridData() {
		...
		Page<Record> page;
		LoginModel loginModel = getSessionAttr("loginModel");
		page = T_ThingInfo.dao.getallGridPage(pageNumber, pageSize, code, name, loginModel, "1");
		List<Record> list = page.getList();
		int totalCount = page.getTotalRow();
		// 调用JsonUtil函数返回datagrid表格json数据
		String jsonStr = JsonUtil.getGridData(list, totalCount);
	...
	public void save() {
		try {
			...
			T_Sys_Operation_Log.dao.insert(success, userid.toString(), "Main/todo/todoadd", methodname, ti, getRequest());
			toDwzText(success, alt, "", "todoDialog", "todoallGrid", "closeCurrent");
		} catch(Exception e) {
	...
	public void geteventbyname() {
		...
		Integer pageSize = getParaToInt("rows");
		Integer pageNumber = getParaToInt("page");
		Page<Record> event = T_Bus_EventInfo.dao.getalleventlist(pageSize, pageNumber, ename);
