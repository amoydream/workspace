...
public class TroubleInfoController extends BaseController {
	...
	public void upd() {
		...
		if(hidtrub.get("cdeptid")!=null){
		String deptid=hidtrub.get("cdeptid").toString();
		if(!login.getIsAdmin() && !deptid.equals(login.getOrgId().toString())){
			toDwzText(false, "只能修改本部门创建的记录，请检查！", "", "", "protectobjGrid", "closeCurrent");
			return;
		}
		}
		render("update.jsp");
	...
	public void save() {
		try {
			...
			if(act.equals("upd")) {
				...
				hidtrub.set("id", AutoId.nextval(hidtrub)).set("isvail", "0").set("cuserid", userid).set("cdeptid", loginModel.getOrgId());
				hidtrub.save();
				success = true;
				alt = "保存成功！";
	...
	public void del() {
		...
		try {
			if(!loginModel.getIsAdmin()){
				List<T_Bus_Hidtrub> hidlist=T_Bus_Hidtrub.dao.getListByids(ids);
				if(hidlist!=null){
					for(T_Bus_Hidtrub hid:hidlist){
						if(hid.get("cdeptid")!=null){
						String deptid=hid.get("cdeptid").toString();
