...
public class ProtectObjController extends BaseController {
	...
	public void protectview() {
		...
		if(obj.get("cdeptid")!=null){
			String deptid=obj.get("cdeptid").toString();
	...
	public void protectupd() {
		...
		LoginModel login = getSessionAttr("loginModel");
		if(defenceobj.get("cdeptid")!=null){
		String deptid=defenceobj.get("cdeptid").toString();
		if(!login.getIsAdmin() && !deptid.equals(login.getOrgId().toString())){
	...
	public void save() {
		try {
			...
			if(act.equals("upd")) {
				...
				bdo.set("defobjid", id).set("cuserid", userid).set("cdeptid", loginModel.getOrgId());
				bdo.save();
				success = true;
				alt = "保存成功！";
	...
	public void protectdel() {
		...
		List<T_Bus_DefenceObj> pojlist=T_Bus_DefenceObj.dao.getListByids(ids);
		if(pojlist!=null){
			for(T_Bus_DefenceObj defenceobj:pojlist){
				if(defenceobj.get("cdeptid")!=null){
				String deptid=defenceobj.get("cdeptid").toString();
