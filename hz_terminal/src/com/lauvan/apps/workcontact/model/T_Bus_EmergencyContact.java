...
public class T_Bus_EmergencyContact extends Model<T_Bus_EmergencyContact> {
	...
	public Page<Record> getContactPage(Integer pageSize, Integer pageNumber, String bookids, String personids) {
		...
		str.append(" from (");
		if(bookids != null && !"".equals(bookids)) {
			str.append("select 'b_'||bo_id as id,u.user_name as name,decode(b.bo_position,null,null,(" + "select to_char(wmsys.wm_concat(a1.p_name)) from" + "(select * from t_sys_parameter start with  p_acode = 'EMPOSITION' connect by prior id=sup_id) a1 " + "where ','||b.bo_position||',' like '%,'||a1.p_acode||',%'" + ")) as position,bo_worknumber as worknumber,bo_homenumber as homenumber," + "bo_mobile as mobile,bo_fax as fax,bo_email as email,bo_address as address" + " from t_bus_contactbook b ,t_sys_user u where b.bo_userid=u.user_id and b.bo_id in(" + bookids + ")");
			str.append(" union ");
		}
		str.append("select 'p_'||p_id as id,p_name as name,decode(p.p_position,null,null,(" + "select to_char(wmsys.wm_concat(a1.p_name)) from" + "(select * from t_sys_parameter start with  p_acode = 'RPOSITION' connect by prior id=sup_id) a1 " + "where ','||p.p_position||',' like '%,'||a1.p_acode||',%'" + ")) as position,p_worknumber as worknumber,p_homenumber as homenumber,p_mobile as mobile," + "p_fax as fax,p_email as email,p_address as address " + "from t_bus_organperson p where p_id in(" + personids + "))");
