...
public class ContactBookController extends BaseController {
	...
	public void save() {
		...
		if((dept_id!=null&&dept_id.intValue()!=orgid)||(model.getNumber("bo_deptid")!=null&&model.getNumber("bo_deptid").intValue()!=orgid)){
			msg = "登录用户不能编辑非具体所属机构和非所属机构人员的通讯录！";
	 }else{
		 String regEx="[`~!@#$%^&*()+=|{}':;',\\[\\].<>/?~！@#￥%……&*（）——+|{}【】‘；：”“’。，、？]";  
		...
		if("true".equals(fromDepart)) {
			if(success) {
				toDwzText(success, "保存成功！", "departcontactTree", "departDialog", "", "closeCurrent", d_pid);
			} else {
				if(StringUtils.isNotBlank(msg)){
				toDwzText(success, msg, "", "", "", "");	
				}else{
