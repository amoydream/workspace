...
public class MasSms {
	private static String					apicode		= "hzyj";
	...
	public static long send(String senderid, String[] tels, String content, String type) {
		try {
			...
			if(result==-9){
				init();
				result = client.sendSM(apicode, loginName, loginPwd, tels, content, maxsn);
			}
			if(tels.length != 0) {
				for(String tel : tels) {
	...
	public static void getRPT() {
		try {
			...
			if(report=="-9"){
				init();
				report = client.recvRPT(apicode, loginName, loginPwd);
			}
					...
					if(rpt != null && rpt.length() > 10) {
						...
						if(mas != null) {
							if(mas.getStr("type").equals("00A")) {
								...
								sd.update();
								//更新关联事件的发送短信
								T_Bus_SmsSendRD sd_r = T_Bus_SmsSendRD.dao.getBySmsId(smid, mobile);
								if(sd_r!=null){
									sd_r.set("sendstate", sd.getStr("send_state"));
									sd_r.update();
									//事件回执
									T_Bus_SmsRevRD rd = new T_Bus_SmsRevRD();
									rd.set("id", AutoId.nextval(rd)).set("smsid", smid).set("callno", mobile).set("revtime", rpttime).set("smsdata", rptdesc);
	...
	public static void getMo() {
		try {
			...
			if(moinfo=="-9"){
				init();
				moinfo = client.recvMo(apicode, loginName, loginPwd);
			}
			T_Sys_Parameter p = T_Sys_Parameter.dao.getByCode("SMSTEMPDZ", "UPDZ");
			Date d = new Date();
			SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMddHHmmss");
			...
			if(moinfo.indexOf("<sms>") != -1) {
				...
				for(String mo : mos) {
					if(mo != null && mo.length() > 10) {
						...
						rev.save();
						//插入关联事件市领导批示
						T_Bus_Moblie_Rev.dao.moRelatEvent(smid, content, mobile, motime);
					}
				}
			}
			file.delete();
	...
	public static void init(){
		String endpoint = attrMap.get("smsip");
		String ip = attrMap.get("masdbhost");
		String dbName = attrMap.get("masdbname");
		String dbPort = attrMap.get("masdbport");
		String loginName = attrMap.get("smsaccount");
		String loginPwd = attrMap.get("smspwd");
		String result = "";
		...
		try {
			...
			//sb.append("http://");
			//sb.append(ip).append(":").append(webPort).append("/axis/services/SMsg?wsdl");
			//String endpoint = sb.toString();
			Service service = new Service();
			Call call = (Call) service.createCall();
			call.setTargetEndpointAddress(new java.net.URL(endpoint));
			call.setOperationName("init");
			result = String.valueOf((call.invoke(new Object[] { ip, dbName, dbPort, loginName, loginPwd })));
