...
public class FireDeptController extends BaseController {
		...
		try {
			...
			if(success) {
				if("0".equals(getPara(0)) || "0".equals(firedept.get("superdept_id").toString())) {
					...
					renderText("{\"success\":true, \"msg\":\"保存成功！\",\"dialogid\":\"firedeptDialog\", \"gridid\":\"firedept_data\", \"treeObj\":\"firedepttree\",\"reloadid\":" + (getPara(0) == null ? firedept.get("superdept_id") : getPara(0)) + ",\"idkey\":\"d_id\"}");
				}
			} else {
	...
	public void getTreeData() {
		...
		List<T_Bus_FireDept> fireList = T_Bus_FireDept.dao.getListById(pid);
		renderJson(fireList);
	}
