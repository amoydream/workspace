...
public class T_Bus_Repertory extends Model<T_Bus_Repertory> {
	...
	public Page<Record> getPage(Integer pageSize, Integer pageNum, String name, String jbCode, String djCode) {
		...
		StringBuffer sql = new StringBuffer("from T_BUS_REPERTORY rep left join (select p2.* from t_sys_parameter p1,t_sys_parameter p2 where p1.id=p2.sup_id and p1.p_acode='" + (jbCode.isEmpty() ? "" : jbCode) + "') jb on rep.levelcode=jb.p_acode" + " left join (select p2.* from t_sys_parameter p1,t_sys_parameter p2 where p1.id=p2.sup_id and p1.p_acode='" + (djCode.isEmpty() ? "" : djCode) + "') dj on rep.deflevelcode=dj.p_acode" + " left join t_sys_department dept on rep.organid =dept.d_id where 1=1");
	...
	public T_Bus_Repertory getRepertoryByRepid(BigDecimal rep_id) {
		return dao.findFirst("select * from t_bus_repertory where rep_id=?", rep_id);
	}
