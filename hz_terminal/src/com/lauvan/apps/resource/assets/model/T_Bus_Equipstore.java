...
public class T_Bus_Equipstore extends Model<T_Bus_Equipstore> {
	...
	public Page<Record> getPage(Integer pageSize, Integer pageNum, String typeCode) {
		...
		StringBuffer sql = new StringBuffer("from t_bus_equipstore sto left join ( " + " select equ.*,para1.id p_id,para1.p_name,para2.p_name unitname from T_Bus_Equipname equ left join (select p2.* from t_sys_parameter p1,t_sys_parameter p2 where p1.id=p2.sup_id and p1.p_acode='" + (typeCode.isEmpty() ? "" : typeCode) + "') para1 on equ.type=para1.id" + " left join (select p2.* from t_sys_parameter p1,t_sys_parameter p2 where p1.id=p2.sup_id and p1.p_acode='MAUNIT') para2 on para2.p_acode=equ.measureunit" + " ) equ2 on sto.equipnameid=equ2.eqn_id left join t_bus_organ dept on sto.organid=dept.or_id where 1=1");
