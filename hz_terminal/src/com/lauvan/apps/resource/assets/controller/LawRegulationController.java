...
public class LawRegulationController extends BaseController {
	...
	public void getTreeData() {
		...
		List<T_Sys_Parameter> typelist = T_Sys_Parameter.dao.getChildByAcode("YJLAWRUL");
	...
	public void getComboTree() {
		String jsonStr = "[]";
		try {
			List<Record> list = T_Sys_Parameter.dao.getParamByCode("YJLAWRUL", true);
	...
	public void getGridData() {
		...
		Page<Record> page = Paginate.dao.getPage("t_bus_lawregulation", pageSize, pageNumber, sqlWhere.toString(), "lr_id", "asc");
	...
	public void add() {
		...
		setAttr("nowdate", DateTimeUtil.formatDate(new Date(), DateTimeUtil.Y_M_D_HMS_FORMAT));
		render("add.jsp");
	}
	...
	public void edit() {
		...
		List<T_Attachment> fileList = T_Attachment.dao.getListByIds(lr.getStr("lr_attachmentid"));
		setAttr("userid", login.getUserId());
		setAttr("lawrul", lr);
	...
	public void save() {
		...
		try {
			if("add".equals(act)) {
				...
				attidsArr = (String[])ArrayUtils.addAll(existidsArr, addidsArr);
				attids = StringUtils.join(attidsArr, ",");
				lr.set("lr_attachmentid", attids);
				success = lr.update();
			}
			if(success) {
				toDwzText(true, "保存成功！", "", "lawrulDialog", "lawrulGrid", "closeCurrent");
			} else {
