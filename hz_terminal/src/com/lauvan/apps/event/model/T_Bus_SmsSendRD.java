...
public class T_Bus_SmsSendRD extends Model<T_Bus_SmsSendRD> {
	...
	public Page<Record> getPageByEventid(Integer pageSize, Integer pageNumber, String eventid) {
		...
		String sql = "select s.*,decode(s.sendstate,'V','发送中','T','成功','F','失败') as smsstate,e.ev_name" + ",decode(s.callname,null,null,(select v.deptname from vw_address v where s.callno = v.telphone and rownum=1)) as deptname" + ",case when (select  count(r.id) from t_bus_smsrevrd r where r.smsid=s.smsid and r.callno=s.callno)>0" + " then '有回执' else '无回执' end as isrev " + ",decode(s.smsid,null,null,(select r.smsdata from t_bus_smsrevrd r where r.smsid=s.smsid and r.callno=s.callno))" + " as bak_rd ";
