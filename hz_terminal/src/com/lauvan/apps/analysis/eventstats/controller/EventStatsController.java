...
public class EventStatsController extends BaseController {
	...
	public void statsResult() {
		...
		List<Record> list = T_Bus_EventInfo.dao.statsEvent(sdate, edate, xval, yval, dlval);
		List<Record> xlist = T_Bus_EventInfo.dao.groupBy(sdate, edate, xval);
		List<Record> ylist = T_Bus_EventInfo.dao.groupBy(sdate, edate, yval);
		setAttr("list", list);
		setAttr("xlist", xlist);
		setAttr("ylist", ylist);
