...
public class AutoViewController extends BaseController {
	...
	public void save() {
		...
		try {
			...
			if(packpath != null && !"".equals(packpath)) {
				...
				AutoCreate.createController(file2, cname.replaceFirst(cname.substring(0, 1), cname.substring(0, 1).toUpperCase()), packpath, name, jsppath, "Main/" + cname, view.getStr("view_name"));
			}
		} catch(Exception e) {
			success = false;
			e.printStackTrace();
	...
	public void getGridUI(StringBuffer str, List<T_AutoAttr_Ext> gridlist, String name) {
		...
		String grid = UIutils.getDatagridUI(name + "_grid", fileds.substring(1).split(","), ftexts.substring(1).split(","), widths.substring(1).split(","), codes.substring(1).split(","));
		str.append(grid);
	}
	 ...
	 * */
	public void getFormUI(StringBuffer str, List<T_AutoAttr_Ext> formlist, String id, Integer num, String act, String name, String var) {
		//默认一行2个属性
		if(num == null) {
			num = 2;
		}
