	...
	function importClick(){
		...
		var attrArray={
				...
				buttons: [{
					...
					id : 'importOrgBtnid',
					handler:function(){
						organimport_dialogSubmit();
					}
				},{
					text:'关闭',
					iconCls:'icon-no',
					handler:function(){
						$("#importOrgDialog").dialog('close');
					}
				}]
		};
		$.lauvan.openCustomDialog("importOrgDialog", attrArray, organimport_dialogSubmit,'organ_form');
	}
	
	
	function organimport_dialogSubmit(){
  		$('#organ_form').form('submit',{
  			onSubmit:function(param){
  				var orgnamenum=document.getElementById("orgnamenum").value;
  				var telnum=document.getElementById("telnum").value;
  				var sonamenum=document.getElementById("sonamenum").value;
  				...
  				if(orgnamenum==""||telnum==""||sonamenum==""||faxnum==""||emailnum==""||addressnum==""){
  					$.messager.alert('错误','存在必填项未填！','error');
						return false;	
  				}else{
  					if(isNaN(orgnamenum)||isNaN(telnum)||isNaN(sonamenum)||isNaN(faxnum)||isNaN(emailnum)||isNaN(addressnum)){
  						$.messager.alert('错误','输入有误，请检查！','error');
  						return false;
  					}
  				}
  				if(orgnamenum<1||telnum<1||sonamenum<1||faxnum<1||emailnum<1||addressnum<1){
  					$.messager.alert('错误','输入的列数不得小于1！','error');
						return false;
  				}
  				$("#importOrgBtnid").linkbutton('disable');
  			},
			success:function(result){
				...
				if(obj.success){
					...
					$('#_organcontactlist_data').datagrid('clearSelections');
					$('#_organcontactlist_data').datagrid('clearChecked');
					$('#_organcontactlist_data').datagrid('reload'); 
					var msg = obj.msg;
					if(obj.errorStr!=''){
		...
		<table id="_organcontactlist_data" cellspacing="0" cellpadding="0">
			<thead>
				<tr>
					...
					<th field="OR_WORKNUMBER" width="100">办公电话</th>
