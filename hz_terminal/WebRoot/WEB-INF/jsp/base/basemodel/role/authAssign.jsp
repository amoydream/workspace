...
<table id="treegrid" class="easyui-treegrid" style="width:100%;"
		...
		<thead>
			...
			<!-- <th data-options="field:'name'" width="40%">菜单</th> -->
				 <th data-options="field:'name',formatter:formatMenu" width="40%">菜单</th>
...
function formatMenu(value,row,field){
	if (value) {
        var name = value;
        var id = row['id'];
        var pid=row['pid'];
        var s= '<label><input type="checkbox" onchange="menucheck('+id+')" parentId="m_'+pid+'" id="m_'+ id +'" value="' + id + '"  >' + name + '</label>&nbsp;&nbsp;&nbsp;';
        }
    return s;
}
function menucheck(id){
	var flag=$("input[id='m_"+id+"']")[0].checked ;
	$("input:checkbox[parentId='"+id+"']").each(function(){
		this.checked=flag;
	});
	 $("input:checkbox[parentId='m_"+id+"']").each(function(){
		this.checked=flag;
		$("input:checkbox[parentId='"+this.id.replace("m_","")+"']").each(function(){
			this.checked=flag;
		});
		menucheck(this.id.replace("m_",""));
	}); 
}
