...
var recer = {};
recer.MAIL = sender.substring(sender.indexOf("<")+1,sender.indexOf(">"));
recer.SMSNAME = sender.substring(0,sender.indexOf("<"));
var mt = [recer];
$("#_mailto").val(recer.MAIL);
$("#_mailtoname").val(recer.SMSNAME);
	
var editor_em1;
var content = $("#emailcontent").val();
var html = "<br><br>";
    html+= "<div style='font-size: 12px;font-family: Arial Narrow;padding:2px 0 2px 0;'>------------------ 原始邮件 ------------------</div>";
    html+= "<div style='font-size: 12px;background:#efefef;padding:8px;'>";
    html+= "<div><b>发件人:</b>"+'${mail.sender}'+"</div>";
    html+= "<div><b>发送时间:</b>"+'${mail.marktime}'+"</div>";
    html+= "<div><b>收件人:</b>"+'${mail.address_to}'+"</div>";
    html+= "<div><b>主题:</b>"+'${mail.subject}'+"</div></div><br><br>";  
$("#emailcontent").val(html+content);
...
$(document).ready(function(){
    ...
    console.info(mt)
    $('#selected_tagger_mailTO').tagger('setTags',mt);
...
<div >
	    ...
	    <table   class="sp-table" width="100%" cellpadding="0" cellspacing="0">
	    		<tr>
	    			...
	    			<td ><input  type="text" class="easyui-textbox" name="t_Bus_Mail_To.subject" value='回复：${mail.subject}'></td>
	    		</tr>
		    	<tr>
		    	...
		    	<c:if test="${!empty fjlist}">
		    		...
		    		<a title='请点击另存为' target='_blank' href='<%=basePath %>Main/attachment/downloadFJ/${fjlist.id}'>${fjlist.name}<a/>${fjlist.size}<a href='javascript:deleteEMFile(${fjlist.id});'><img src='<%=basePath %>plugins/uploadify/cancel.png' height='13' align='middle'/></a>
		    		</div>
		    		</c:forEach>
		    	</c:if>
		    	</div>
