...
function tran(str,f){
	...
	for(var i=0; i<str.length;i++){
		address[i] = str[i].substring(str[i].indexOf("<")+1,str[i].indexOf(">"));
		name[i] = str[i].substring(0,str[i].indexOf("<"));
	}
	if(f==0){return address;}
	if(f==1){return name;}
}
var addrto = "${mail.address_to}".split(",");
var addrcc = "${mail.address_cc}".split(",");
var tag_arr_to = [];
var tag_arr_cc = [];
for(var i=0; i<addrto.length; i++){
	var obj = {};
	obj.MAIL = tran(addrto,0)[i];
    obj.SMSNAME = tran(addrto,1)[i];
	tag_arr_to.push(obj);
}
for(var i=0; i<addrcc.length; i++){
	var obj = {};
	obj.MAIL = tran(addrcc,0)[i];
    obj.SMSNAME = tran(addrcc,1)[i];
	tag_arr_cc.push(obj);
}
var tomail = tran(addrto,0).join(",")
var toname = tran(addrto,1).join(",")
var ccmail = tran(addrcc,0).join(",")
var ccname = tran(addrcc,1).join(",")
$("#_mailto").val(tomail);
$("#_mailtoname").val(toname);
$("#_mailcc").val(ccmail);
$("#_mailccname").val(ccname);
...
$(document).ready(function(){
    ...
    $('#selected_tagger_mailTO').tagger('setTags',tag_arr_to);
    ...
    var tagger_optionscc = {
        ...
        $('#selected_tagger_mailCC').tagger('setTags',tag_arr_cc);
...
<div >
	    ...
	    <div >
	     ...
	     <input type="hidden" name="t_Bus_Mail_To.id" value='${mail.id}' />
	    <table   class="sp-table" width="100%" cellpadding="0" cellspacing="0">
	    		<tr>
	    			<td class="sp-td1">主题：</td>
	    			<td ><input  type="text" class="easyui-textbox" name="t_Bus_Mail_To.subject" value='${mail.subject}'></td>
	    		</tr>
		    	<tr>
		    	...
		    	<div id="mailFJfileList" style="width: 900px;">
		    	<c:if test="${!empty fjlist}">
		    		<c:forEach items="${fjlist}" var="fjlist">
		    		<div id='emTXfile_${fjlist.id}' style='height:25px;line-height:25px;font-size:12px;'>
		    		<span style='display:none'><input type='checkbox' name='emfjid' value='${fjlist.id}' checked/></span>
		    		<a title='请点击另存为' target='_blank' href='<%=basePath %>Main/attachment/downloadFJ/${fjlist.id}'>${fjlist.name}(${fjlist.size})<a/><a href='javascript:deleteEMFile(${fjlist.id});'><img src='<%=basePath %>plugins/uploadify/cancel.png' height='13' align='middle'/></a>
		    		</div>
		    		</c:forEach>
		    	</c:if>
		    	</div>
		    	...
		    	<td colspan="2">
		    		<textarea id="emailcontent" name="t_Bus_Mail_To.content" class="textarea" 
		    		  style="width: 100%;height: 220px;">${mail.content }</textarea>
		    	</td>
		    	</tr>
	    </table>
	     </form>
