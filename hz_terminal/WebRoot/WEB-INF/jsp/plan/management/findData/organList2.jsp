...
<script type="text/javascript">
	...
	function zTreeOnClick_orgper2(event, treeId, treeNode) {
		$('#orgperGrid2').datagrid({url:'<%=basePath%>Main/planMg/getSmsList/'+treeNode.id});
	};
	$(document).ready(function(){
		...
		var attrArray={
				...
				onLoadSuccess:function(data){
					$.each(data.rows, function(index, item){
						if((","+'${groupOrg.itemid}'+",").indexOf(","+item.ID+",")>=0){
							$('#orgperGrid2').datagrid('checkRow', index);
						}
					});
				},
				...
				orgper.rows = rows;
				orgper.total = total;
				$("#orgperSelect2").datalist("loadData",orgper);
			},
		...
		$.lauvan.dataGrid("orgperGrid2",attrArray);
		$("#orgperSelect2").datalist({
			fit:true,
			valueField:'ID',
			textField: 'SMSNAME'	
		});
		$.fn.zTree.init($("#orgperTree2"), setting_orgper2, zNodes_orgper2);
		zTree_orgper2 = $.fn.zTree.getZTreeObj('orgperTree2');
		zTree_orgper2.selectNode(zTree_orgper2.getNodeByParam("id", '${apId}', null));
	});
</script>
<div class="easyui-layout"  data-options="fit:true">
<div data-options="region:'west',border:false" style="width: 200px;">
	<ul id="orgperTree2" class="ztree"></ul>
</div>
<div data-options="region:'center',border:false">
	<table id="orgperGrid2" cellspacing="0" cellpadding="0"> 
				    <thead> 
				        <tr> 
	 ...
	 <form id="orgperform" method="post" action="<%=basePath %>Main/planMg/save" style="width:100%;">
		 <input type="hidden" name="t_Bus_PlanItem.id" value="${empty groupOrg.id?0: groupOrg.id}"/>
		  <input type="hidden" name="t_Bus_PlanItem.pid" value="${empty groupOrg.pid?0: groupOrg.pid}"/>
		  <input type="hidden" name="act" value="orgper_organ"/>
	</form>
		<div id="orgperSelect2" class="easyui-datalist" title="已选择机构/人员" lines="true" >
